...
def UNIFORM(SHAPE, SCALE=0.5):
    ...
    """
    return np.random.uniform(size=SHAPE, low=-SCALE,
                             high=SCALE)  # RETURN A SAMPLE (OR SAMPLES) FROM THE “UNIFORM” DISTRIBUTION. IF SHAPE IS AN INTEGER, THEN IT IS TREATED AS A TUPLE OF LENGTH ONE, AND THE METHOD RETURNS A SINGLE NUMPY FLOATING POINT NUMBER. IF SHAPE IS A TUPLE OF INTEGERS, THEN IT IS TREATED AS A VECTOR OF LENGTH SHAPE, AND THE METHOD RETURNS A NUMPY FLOATING POINT VECTOR OF THAT SHAPE. IF SHAPE IS A TUPLE, THEN A FLOATING POINT TENSOR OF SHAPE SHAPE AND STANDARD DEVIATION SCALE IS RETURNED. IF SCALE IS NONE (THE DEFAULT), THEN SAMPLES ARE DRAWN FROM A STANDARD NORMAL DISTRIBUTION (I.E., STANDARD DEVIATION IS 1).
...
def __GLOROT_FAN__(SHAPE):
    ...
    """
    assert len(
        SHAPE) >= 2  # ASSERTION TO CHECK IF THE LENGTH OF SHAPE IS GREATER THAN OR EQUAL TO 2.
    if len(SHAPE) == 4:  # IF THE LENGTH OF SHAPE IS 4.
        # RETURN THE PRODUCT OF ARRAY ELEMENTS OVER A GIVEN AXIS.
        RECEPTIVE_FIELD_SIZE = np.prod(SHAPE[2:])
        # FAN IN = SHAPE[1] * RECEPTIVE_FIELD_SIZE.
        FAN_IN = SHAPE[1] * RECEPTIVE_FIELD_SIZE
        # FAN OUT = SHAPE[0] * RECEPTIVE_FIELD_SIZE.
...
def GLOROT_NORMAL(SHAPE):
    ...
    ARRAY : NUMPY ARRAY
        ARRAY OF SHAPE SHAPE WITH VALUES DRAWN FROM A NORMAL DISTRIBUTION CENTERED AT 0 WITH STANDARD DEVIATION SCALE.
    """
    FAN_IN, FAN_OUT = __GLOROT_FAN__(
        SHAPE)  # FAN IN AND FAN OUT FOR THE GLOROT INITIALIZATION.
    # S = SQRT(2.0 / (FAN_IN + FAN_OUT)).
    S = np.sqrt(2.0 / (FAN_IN + FAN_OUT))
    return NORMAL(SHAPE,
                  S)  # RETURN A SAMPLE (OR SAMPLES) FROM THE “STANDARD NORMAL” DISTRIBUTION. IF SHAPE IS AN INTEGER, THEN IT IS TREATED AS A TUPLE OF LENGTH ONE, AND THE METHOD RETURNS A SINGLE NUMPY FLOATING POINT NUMBER. IF SHAPE IS A TUPLE OF INTEGERS, THEN IT IS TREATED AS A VECTOR OF LENGTH SHAPE, AND THE METHOD RETURNS A NUMPY FLOATING POINT VECTOR OF THAT SHAPE. IF SHAPE IS A TUPLE, THEN A FLOATING POINT TENSOR OF SHAPE SHAPE AND STANDARD DEVIATION SCALE IS RETURNED. IF SCALE IS NONE (THE DEFAULT), THEN SAMPLES ARE DRAWN FROM A STANDARD NORMAL DISTRIBUTION (I.E., STANDARD DEVIATION IS 1).
...
def GLOROT_UNIFORM(SHAPE):
    ...
    FAN_IN, FAN_OUT = __GLOROT_FAN__(
        SHAPE)  # FAN IN AND FAN OUT FOR THE GLOROT INITIALIZATION.
    # S = SQRT(6.0 / (FAN_IN + FAN_OUT)).
    S = np.sqrt(6.0 / (FAN_IN + FAN_OUT))
    return UNIFORM(SHAPE,
                   S)  # RETURN A SAMPLE (OR SAMPLES) FROM THE “UNIFORM” DISTRIBUTION. IF SHAPE IS AN INTEGER, THEN IT IS TREATED AS A TUPLE OF LENGTH ONE, AND THE METHOD RETURNS A SINGLE NUMPY FLOATING POINT NUMBER. IF SHAPE IS A TUPLE OF INTEGERS, THEN IT IS TREATED AS A VECTOR OF LENGTH SHAPE, AND THE METHOD RETURNS A NUMPY FLOATING POINT VECTOR OF THAT SHAPE. IF SHAPE IS A TUPLE, THEN A FLOATING POINT TENSOR OF SHAPE SHAPE AND STANDARD DEVIATION SCALE IS RETURNED. IF SCALE IS NONE (THE DEFAULT), THEN SAMPLES ARE DRAWN FROM A STANDARD NORMAL DISTRIBUTION (I.E., STANDARD DEVIATION IS 1).
