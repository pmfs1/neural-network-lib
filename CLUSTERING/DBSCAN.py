...
def DBSCAN(X, MIN_SAMPLES=5, EPSILON=0.5):
    ...
    LABELS = np.zeros(N_SAMPLES, dtype=int) # ARRAY OF CLUSTER LABELS FOR EACH SAMPLE
    ...
    def __EXPAND_CLUSTER__(VISITED, DISTANCES, NEIGHBOURS, CLUSTER_ID):
        ...
        for NEIGHBOUR_INDEX in NEIGHBOURS: # ITERATE OVER THE NEIGHBOURS OF THE CURRENT SAMPLE
            if not VISITED[NEIGHBOUR_INDEX]: # IF THE NEIGHBOUR HAS NOT BEEN VISITED
                VISITED[NEIGHBOUR_INDEX] = True # MARK THE NEIGHBOUR AS VISITED
                NEIGHBOUR_NEIGHBOURS = DISTANCES[NEIGHBOUR_INDEX, 1:] # GET THE NEIGHBOUR'S NEIGHBOURS
                NUM_NEIGHBOUR_NEIGHBOURS = len(NEIGHBOUR_NEIGHBOURS) # GET THE NUMBER OF NEIGHBOUR'S NEIGHBOURS
                if NUM_NEIGHBOUR_NEIGHBOURS >= MIN_SAMPLES: # IF THE NEIGHBOUR HAS ENOUGH NEIGHBOURS TO BE A CORE POINT
                    NEIGHBOURS.extend(NEIGHBOUR_NEIGHBOURS) # ADD THE NEIGHBOUR'S NEIGHBOURS TO THE LIST OF NEIGHBOURS
                if LABELS[NEIGHBOUR_INDEX] == 0: # IF THE NEIGHBOUR DOES NOT HAVE A CLUSTER LABEL ASSIGNED
                    LABELS[NEIGHBOUR_INDEX] = CLUSTER_ID # ASSIGN THE CURRENT CLUSTER IDENTIFIER

    DISTANCES = PAIRWISE_DISTANCES(X) # CALCULATE THE PAIRWISE DISTANCES BETWEEN THE SAMPLES
    VISITED = np.zeros(N_SAMPLES, dtype=bool) # INITIALISE THE VISITED STATUS OF ALL SAMPLES TO FALSE
    for i in range(N_SAMPLES): # ITERATE OVER THE SAMPLES
        ...
        NEIGHBOURS = np.where(DISTANCES[i, :] <= EPSILON)[0] # GET THE NEIGHBOURS OF THE CURRENT SAMPLE
        ...
        if NUM_NEIGHBOURS < MIN_SAMPLES: # IF THE SAMPLE DOES NOT HAVE ENOUGH NEIGHBOURS TO BE A CORE POINT
            ...
            __EXPAND_CLUSTER__(VISITED, DISTANCES, NEIGHBOURS.tolist(), CLUSTER_ID) # EXPAND THE CLUSTER BY ADDING CONNECTED SAMPLES
