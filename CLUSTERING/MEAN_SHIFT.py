    ...
    for _ in range(MAX_ITERATIONS): # ITERATE UNTIL CONVERGENCE OR MAXIMUM ITERATIONS
        ...
        for CENTROID in CENTROIDS: # FOR EACH CENTROID
            ...
            for _X in X: # FOR EACH DATA POINT
                if __EUCLIDEAN_DISTANCE__(_X, CENTROID) <= BANDWIDTH: # IF DATA POINT IS WITHIN BANDWIDTH
                    WITHIN_BANDWIDTH.append(_X) # ADD DATA POINT TO WITHIN BANDWIDTH
            if WITHIN_BANDWIDTH: # IF THERE ARE DATA POINTS WITHIN BANDWIDTH
                NEW_CENTROID = np.mean(WITHIN_BANDWIDTH, axis=0) # CALCULATE NEW CENTROID
                NEW_CENTROIDS.append(NEW_CENTROID) # ADD NEW CENTROID TO NEW CENTROIDS
        if len(NEW_CENTROIDS) == len(CENTROIDS) and np.allclose(CENTROIDS, NEW_CENTROIDS): # IF CONVERGENCE IS ACHIEVED
