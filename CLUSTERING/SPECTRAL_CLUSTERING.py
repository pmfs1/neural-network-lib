...
def GAUSSIAN_AFFINITY_MATRIX(X):
    ...
    DISTANCES = np.linalg.norm(X[:, np.newaxis] - X, axis=2) # CALCULATE THE PAIRWISE DISTANCES BETWEEN DATA POINTS
    SIGMA = np.median(DISTANCES) # CALCULATE THE MEDIAN OF THE PAIRWISE DISTANCES
    AFFINITY_MATRIX = np.exp(-DISTANCES ** 2 / (2 * SIGMA ** 2)) # CONSTRUCT THE AFFINITY MATRIX USING THE GAUSSIAN SIMILARITY MEASURE
    return AFFINITY_MATRIX # RETURN THE COMPUTED GAUSSIAN AFFINITY MATRIX
...
def EPSILON_NEIGHBOURHOOD_AFFINITY_MATRIX(X, EPSILON=0.5):
    ...
    DISTANCES = np.linalg.norm(X[:, np.newaxis] - X, axis=2) # CALCULATE THE PAIRWISE DISTANCES BETWEEN DATA POINTS
    AFFINITY_MATRIX = np.where(DISTANCES <= EPSILON, 1, 0) # CONSTRUCT THE AFFINITY MATRIX BASED ON THE EPSILON NEIGHBOURHOOD APPROACH
    return AFFINITY_MATRIX # RETURN THE COMPUTED EPSILON NEIGHBOURHOOD AFFINITY MATRIX
...
def COSINE_SIMILARITY_AFFINITY_MATRIX(X):
    ...
    def __COSINE_SIMILARITY__(VECTOR_1, VECTOR_2):
        ...
        DOT_PRODUCT = np.dot(VECTOR_1, VECTOR_2.T) # DOT PRODUCT CALCULATION
        NORM_PRODUCT = np.linalg.norm(VECTOR_1) * np.linalg.norm(VECTOR_2) # MAGNITUDE CALCULATION
        return DOT_PRODUCT / NORM_PRODUCT # COSINE SIMILARITY

    NUM_SAMPLES = len(X) # NUMBER OF SAMPLES
    AFFINITY_MATRIX = np.zeros((NUM_SAMPLES, NUM_SAMPLES)) # INITIALIZE THE AFFINITY MATRIX
    for i in range(NUM_SAMPLES): # ITERATE THROUGH THE SAMPLES (ROWS)
        for j in range(i, NUM_SAMPLES): # ITERATE THROUGH THE SAMPLES (COLUMNS)
            SIMILARITY = __COSINE_SIMILARITY__(X[i].reshape(1, -1), X[j].reshape(1, -1)) # CALCULATE THE COSINE SIMILARITY BETWEEN THE SAMPLES
            AFFINITY_MATRIX[i, j] = SIMILARITY # UPDATE THE AFFINITY MATRIX WITH THE SIMILARITY VALUE
            AFFINITY_MATRIX[j, i] = SIMILARITY # UPDATE THE AFFINITY MATRIX WITH THE SIMILARITY VALUE (SYMMETRIC)
    return AFFINITY_MATRIX # RETURN THE AFFINITY MATRIX
...
def SPECTRAL_CLUSTERING(X, K, AFFINITY_MATRIX_FUNCTION=GAUSSIAN_AFFINITY_MATRIX):
    ...
    def __COMPUTE_NORMALIZED_LAPLACIAN__(AFFINITY_MATRIX):
        ...
        DEGREE_MATRIX = np.diag(np.sum(AFFINITY_MATRIX, axis=1)) # SUM THE ROWS OF THE AFFINITY MATRIX TO OBTAIN THE DEGREE MATRIX
        LAPLACIAN_MATRIX = DEGREE_MATRIX - AFFINITY_MATRIX # SUBTRACT THE AFFINITY MATRIX FROM THE DEGREE MATRIX TO OBTAIN THE LAPLACIAN MATRIX
        SQRT_INVERSE_DEGREE_MATRIX = np.sqrt(np.linalg.inv(DEGREE_MATRIX)) # CALCULATE THE SQUARE ROOT OF THE INVERSE OF THE DEGREE MATRIX
        NORMALIZED_LAPLACIAN_MATRIX = np.dot(np.dot(SQRT_INVERSE_DEGREE_MATRIX, LAPLACIAN_MATRIX), SQRT_INVERSE_DEGREE_MATRIX) # MULTIPLY THE SQUARE ROOT OF THE INVERSE DEGREE MATRIX BY THE LAPLACIAN MATRIX TWICE TO OBTAIN THE NORMALIZED LAPLACIAN MATRIX
        return NORMALIZED_LAPLACIAN_MATRIX # RETURN THE COMPUTED NORMALIZED LAPLACIAN MATRIX
    ...
    def __SELECT_EIGENVECTORS__(EIGENVALUES, EIGENVECTORS, K):
        ...
        SORTED_IDXS = np.argsort(EIGENVALUES) # OBTAIN THE INDICES THAT WOULD SORT THE EIGENVALUES IN ASCENDING ORDER
        _ = EIGENVALUES[SORTED_IDXS] # SORT THE EIGENVALUES IN ASCENDING ORDER
        SORTED_EIGENVECTORS = EIGENVECTORS[:, SORTED_IDXS] # SORT THE EIGENVECTORS ACCORDING TO THE SORTED EIGENVALUES
        SELECTED_EIGENVECTORS = SORTED_EIGENVECTORS[:, :K] # SELECT THE FIRST `K` EIGENVECTORS
        return SELECTED_EIGENVECTORS # RETURN THE SELECTED EIGENVECTORS

    AFFINITY_MATRIX = AFFINITY_MATRIX_FUNCTION(X) # COMPUTE THE AFFINITY MATRIX
    LAPLACIAN_MATRIX = __COMPUTE_NORMALIZED_LAPLACIAN__(AFFINITY_MATRIX) # COMPUTE THE NORMALIZED LAPLACIAN MATRIX
    EIGENVALUES, EIGENVECTORS = np.linalg.eig(LAPLACIAN_MATRIX) # COMPUTE THE EIGENVALUES AND EIGENVECTORS OF THE NORMALIZED LAPLACIAN MATRIX
    EIGENVECTORS = __SELECT_EIGENVECTORS__(EIGENVALUES, EIGENVECTORS, K) # SELECT THE EIGENVECTORS CORRESPONDING TO THE SMALLEST EIGENVALUES
    LABELS = K_MEANS_CLUSTERING(EIGENVECTORS, K) # PERFORM K-MEANS CLUSTERING ON THE SELECTED EIGENVECTORS TO OBTAIN THE CLUSTER LABELS
