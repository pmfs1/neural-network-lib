...
def GAUSSIAN_AFFINITY_MATRIX(X: np.ndarray) -> np.ndarray:
    ...
    DISTANCES = np.linalg.norm(
        X[:, np.newaxis] - X, axis=2)  # CALCULATE THE PAIRWISE DISTANCES BETWEEN DATA POINTS
    # CALCULATE THE MEDIAN OF THE PAIRWISE DISTANCES
...
def EPSILON_NEIGHBOURHOOD_AFFINITY_MATRIX(X: np.ndarray, EPSILON: float = 0.5) -> np.ndarray:
...
def COSINE_SIMILARITY_AFFINITY_MATRIX(X: np.ndarray) -> np.ndarray:
    ...
    def __COSINE_SIMILARITY__(VECTOR_1: np.ndarray, VECTOR_2: np.ndarray) -> float:
        ...
        NORM_PRODUCT = np.linalg.norm(
            VECTOR_1) * np.linalg.norm(VECTOR_2)  # MAGNITUDE CALCULATION
...
def SPECTRAL_CLUSTERING(X: np.ndarray, K: int, AFFINITY_MATRIX_FUNCTION=GAUSSIAN_AFFINITY_MATRIX) -> np.ndarray:
    ...
    def __COMPUTE_NORMALIZED_LAPLACIAN__(AFFINITY_MATRIX: np.ndarray) -> np.ndarray:
        ...
        DEGREE_MATRIX = np.diag(np.sum(
            AFFINITY_MATRIX, axis=1))  # SUM THE ROWS OF THE AFFINITY MATRIX TO OBTAIN THE DEGREE MATRIX
        # SUBTRACT THE AFFINITY MATRIX FROM THE DEGREE MATRIX TO OBTAIN THE LAPLACIAN MATRIX
        ...
        # MULTIPLY THE SQUARE ROOT OF THE INVERSE DEGREE MATRIX BY THE LAPLACIAN MATRIX TWICE TO OBTAIN THE NORMALIZED LAPLACIAN MATRIX
        NORMALIZED_LAPLACIAN_MATRIX = np.dot(np.dot(
            SQRT_INVERSE_DEGREE_MATRIX, LAPLACIAN_MATRIX), SQRT_INVERSE_DEGREE_MATRIX)
        # RETURN THE COMPUTED NORMALIZED LAPLACIAN MATRIX
    ...
    def __SELECT_EIGENVECTORS__(EIGENVALUES: np.ndarray, EIGENVECTORS: np.ndarray, K: int) -> np.ndarray:
        ...
        SORTED_IDXS = np.argsort(
            EIGENVALUES)  # OBTAIN THE INDICES THAT WOULD SORT THE EIGENVALUES IN ASCENDING ORDER
    ...
    AFFINITY_MATRIX = AFFINITY_MATRIX_FUNCTION(
        X)  # COMPUTE THE AFFINITY MATRIX
    LAPLACIAN_MATRIX = __COMPUTE_NORMALIZED_LAPLACIAN__(
        AFFINITY_MATRIX)  # COMPUTE THE NORMALIZED LAPLACIAN MATRIX
    ...
    # PERFORM K-MEANS CLUSTERING ON THE SELECTED EIGENVECTORS TO OBTAIN THE CLUSTER LABELS
