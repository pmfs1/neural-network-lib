...
def AFFINITY_PROPAGATION(X, MAX_ITERATIONS=200, CONVERGENCE_ITERATIONS=15, DAMPING=0.5):
    ...
    def __CALCULATE_SIMILARITY_MATRIX__(X):
        ...
        S = -np.inf * np.ones((N_SAMPLES, N_SAMPLES)) # INITIALIZE SIMILARITY MATRIX
        for i in range(N_SAMPLES): # ITERATE OVER SAMPLES (I)
            for j in range(N_SAMPLES): # ITERATE OVER SAMPLES (J)
                S[i, j] = -np.linalg.norm(X[i] - X[j]) # CALCULATE SIMILARITY BETWEEN SAMPLES
    ...
    def __UPDATE_RESPONSIBILITIES__(A, S):
        ...
        AS[np.arange(N_SAMPLES), MAX_IDXS] = -np.inf # SET MAXIMUM VALUES TO -INF
        SECOND_MAX_VALUES = np.amax(AS, axis=1) # FIND SAMPLES WITH SECOND HIGHEST INFLUENCE
        R = S - np.repeat(MAX_VALUES[:, np.newaxis], N_SAMPLES, axis=1) # CALCULATE RESPONSIBILITIES
        R[np.arange(N_SAMPLES), MAX_IDXS] = S[np.arange(N_SAMPLES), MAX_IDXS] - SECOND_MAX_VALUES # CALCULATE SELF-RESPONSIBILITIES
    ...
    def __UPDATE_AVAILABILITIES__(R):
        ...
        R_P[np.arange(N_SAMPLES), np.arange(N_SAMPLES)] = R[np.arange(N_SAMPLES), np.arange(N_SAMPLES)] # PRESERVE SELF-RESPONSIBILITIES
        A = np.sum(R_P, axis=0) # CALCULATE AVAILABILITIES
        D_A = np.diag(A) # CREATE DIAGONAL MATRIX USING AVAILABILITY VALUES
        A = np.minimum(A, 0) # SET AVAILABILITIES TO BE MINIMUM OF 0 AND SUM OF POSITIVE RESPONSIBILITIES
        A[np.arange(N_SAMPLES), np.arange(N_SAMPLES)] = np.diag(D_A) # PRESERVE SELF-AVAILABILITIES
    ...
    A = np.zeros((N_SAMPLES, N_SAMPLES)) # INITIALIZE THE AVAILABILITIES MATRIX
    R = np.zeros((N_SAMPLES, N_SAMPLES)) # INITIALIZE THE RESPONSIBILITIES MATRIX
    for _ in range(MAX_ITERATIONS): # ITERATE FOR THE MAXIMUM NUMBER OF ITERATIONS
        ...
        R = __UPDATE_RESPONSIBILITIES__(A, S) # UPDATE THE RESPONSIBILITIES MATRIX
        A = __UPDATE_AVAILABILITIES__(R) # UPDATE THE AVAILABILITIES MATRIX
        if __CHECK_CONVERGENCE__(R, R_OLD, A, A_OLD): # CHECK IF THE ALGORITHM HAS CONVERGED
        ...
        A = DAMPING * A + (1 - DAMPING) * A_OLD # DAMPEN THE AVAILABILITIES MATRIX
        R = DAMPING * R + (1 - DAMPING) * R_OLD # DAMPEN THE RESPONSIBILITIES MATRIX
    CLUSTER_CENTER_IDXS = np.where(np.diag(A + R) > 0)[0] # GET THE INDICES OF THE CLUSTER CENTERS
    LABELS = np.argmax(S[:, CLUSTER_CENTER_IDXS], axis=1) # GET THE LABELS OF THE SAMPLES
    LABELS[CLUSTER_CENTER_IDXS] = np.arange(len(CLUSTER_CENTER_IDXS)) # ASSIGN THE CLUSTER CENTER SAMPLES TO THEIR OWN CLUSTERS
    return CLUSTER_CENTER_IDXS, LABELS # RETURN THE CLUSTER CENTER INDICES AND SAMPLE LABELS