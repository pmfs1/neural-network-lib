...
def AFFINITY_PROPAGATION(X, MAX_ITERATIONS=200, CONVERGENCE_ITERATIONS=15, DAMPING=0.5):
    ...
    # THE `CALCULATE_SIMILARITY_MATRIX` FUNCTION IS RESPONSIBLE FOR COMPUTING THE SIMILARITY MATRIX `S` BASED ON THE INPUT DATA `X`.
    #     1. **INPUT PARAMETER**:
    #         - `X`: THE INPUT DATA MATRIX OF SHAPE `(N_SAMPLES, N_FEATURES)` WHERE `N_SAMPLES` REPRESENTS THE NUMBER OF SAMPLES AND `N_FEATURES` REPRESENTS THE NUMBER OF FEATURES IN THE DATA.
    ...
    def CALCULATE_SIMILARITY_MATRIX(X):
        ...
        THE `CALCULATE_SIMILARITY_MATRIX` FUNCTION COMPUTES THE SIMILARITY MATRIX `S` BY CALCULATING THE NEGATIVE EUCLIDEAN DISTANCE BETWEEN PAIRS OF SAMPLES. THIS MATRIX CAPTURES THE SIMILARITY RELATIONSHIPS BETWEEN SAMPLES AND IS USED AS A FOUNDATION FOR SUBSEQUENT STEPS IN THE AFFINITY PROPAGATION ALGORITHM."""
        N_SAMPLES = X.shape[0]
    ...
    # THE `UPDATE_RESPONSIBILITIES` FUNCTION IS RESPONSIBLE FOR UPDATING THE RESPONSIBILITIES MATRIX `R` IN THE AFFINITY PROPAGATION ALGORITHM.
    #     1. **INPUT PARAMETERS**:
    ...
    #         - CREATE A COPY OF THE OLD RESPONSIBILITIES MATRIX `R` AND STORE IT AS `R_OLD`.
    ...
    # THE `UPDATE_AVAILABILITIES` FUNCTION IS RESPONSIBLE FOR UPDATING THE AVAILABILITIES MATRIX `A` IN THE AFFINITY PROPAGATION ALGORITHM.
    #     1. **INPUT PARAMETER**:
    #         - `R`: THE RESPONSIBILITIES MATRIX OF SHAPE `(N_SAMPLES, N_SAMPLES)`.
    #     2. **AVAILABILITY CALCULATION**:
    #         - CREATE A COPY OF THE OLD AVAILABILITIES MATRIX `A` AND STORE IT AS `A_OLD`.
    ...
    # THE `UPDATE_AVAILABILITIES` FUNCTION CALCULATES THE UPDATED AVAILABILITIES MATRIX `A` BASED ON THE RESPONSIBILITIES MATRIX `R`. IT ENSURES THAT THE SELF-AVAILABILITIES ARE PRESERVED AND COMPUTES THE AVAILABILITY VALUES BY SUMMING UP POSITIVE RESPONSIBILITIES FOR EACH SAMPLE. BY ADJUSTING THE AVAILABILITIES, THE FUNCTION INFLUENCES THE EXEMPLAR SELECTION PROCESS IN THE AFFINITY PROPAGATION ALGORITHM, LEADING TO REFINED CLUSTER ASSIGNMENTS.
    def UPDATE_AVAILABILITIES(R):
        """THE `UPDATE_AVAILABILITIES` FUNCTION IS RESPONSIBLE FOR UPDATING THE AVAILABILITIES MATRIX `A` IN THE AFFINITY PROPAGATION ALGORITHM.
    ...
    # THE `CHECK_CONVERGENCE` FUNCTION IS RESPONSIBLE FOR CHECKING THE CONVERGENCE OF THE AFFINITY PROPAGATION ALGORITHM BY COMPARING THE CURRENT RESPONSIBILITIES AND AVAILABILITIES MATRICES WITH THEIR PREVIOUS ITERATIONS.
    #     1. **INPUT PARAMETERS**:
