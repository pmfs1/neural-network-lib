...
def BIRCH(X, BRANCHING_FACTOR, THRESHOLD=0.5):
    ...
    class BIRCH_NODE:
        ...
        def __init__(self, IS_LEAF=True):
            ...
            self.SUBCLUSTER_CENTERS = [] # EMPTY LIST TO HOLD THE SUBCLUSTER CENTERS ASSOCIATED WITH THE NODE
            self.N_SAMPLES = 0 # COUNT OF THE NUMBER OF DATA POINTS IN THE SUBCLUSTERS ASSOCIATED WITH THE NODE, INITIALLY SET TO 0
            self.PROTOTYPE = None # THE PROTOTYPE (CENTER) OF THE SUBCLUSTER CENTERS ASSOCIATED WITH THE NODE, INITIALLY SET TO NONE
        ...
        def ADD_CHILD(self, CHILD):
            ...
            self.CHILDREN.append(CHILD) # APPEND THE CHILD NODE TO THE `CHILDREN` LIST OF THE CURRENT NODE
        ...
        def UPDATE_SUBCLUSTER_CENTERS(self, POINT):
            ...
            self.SUBCLUSTER_CENTERS.append(POINT) # APPEND THE NEW DATA POINT TO THE `SUBCLUSTER_CENTERS` LIST OF THE CURRENT NODE.
            self.N_SAMPLES += 1 # INCREMENT THE SAMPLE COUNT (`N_SAMPLES`) ASSOCIATED WITH THE NODE BY 1.
        ...
        def COMPUTE_PROTOTYPE(self):
            ...
            self.SUBCLUSTER_CENTERS = np.array(self.SUBCLUSTER_CENTERS) # CONVERT THE `SUBCLUSTER_CENTERS` LIST INTO A NUMPY ARRAY.
            self.PROTOTYPE = np.mean(self.SUBCLUSTER_CENTERS, axis=0) # CALCULATE THE MEAN OF THE SUBCLUSTER CENTERS ALONG EACH DIMENSION TO OBTAIN THE PROTOTYPE.
    ...
    def __INSERT__(POINT, NODE):
        ...
        if NODE.IS_LEAF: # IF THE CURRENT NODE IS A LEAF NODE, UPDATE ITS SUBCLUSTER CENTERS AND SAMPLE COUNT.
            NODE.UPDATE_SUBCLUSTER_CENTERS(POINT) # UPDATE THE SUBCLUSTER CENTERS OF THE LEAF NODE BY ADDING THE CURRENT DATA POINT.
            if NODE.SHOULD_SPLIT(): # IF THE LEAF NODE EXCEEDS THE THRESHOLD, TRIGGER THE SPLITTING PROCESS.
                __SPLIT_NODE__(NODE) # SPLIT THE LEAF NODE INTO MULTIPLE CHILD NODES.
            ...
            for CHILD in NODE.CHILDREN: # ITERATE THROUGH THE CHILDREN OF THE CURRENT NODE.
                DISTANCE = np.linalg.norm(POINT - CHILD.PROTOTYPE) # COMPUTE THE DISTANCE BETWEEN THE DATA POINT AND THE PROTOTYPE OF THE CHILD NODE.
                if DISTANCE < MIN_DISTANCE: # IF THE DISTANCE IS LESS THAN THE MINIMUM DISTANCE, UPDATE THE MINIMUM DISTANCE AND CLOSEST CHILD NODE.
                    MIN_DISTANCE = DISTANCE # UPDATE THE MINIMUM DISTANCE.
                    CLOSEST_CHILD = CHILD # UPDATE THE CLOSEST CHILD NODE.
            __INSERT__(POINT, CLOSEST_CHILD) # RECURSIVELY CALL THE FUNCTION WITH THE CLOSEST CHILD NODE AS THE NEW CURRENT NODE.
    ...
    def __SPLIT_NODE__(NODE):
        ...
        CHILD_THRESHOLD = THRESHOLD / 2 # THE CHILD THRESHOLD IS HALF OF THE OVERALL THRESHOLD.
        SUBCLUSTER_CENTERS = NODE.SUBCLUSTER_CENTERS # THE SUBCLUSTER CENTERS ARE THE SUBCLUSTER CENTERS OF THE NODE.
        NODE.SUBCLUSTER_CENTERS = [] # THE SUBCLUSTER CENTERS OF THE NODE ARE RESET.
        NODE.N_SAMPLES = 0 # THE NUMBER OF SAMPLES IN THE NODE IS RESET.
        for SUBCLUSTER_CENTER in SUBCLUSTER_CENTERS: # FOR EACH SUBCLUSTER CENTER IN THE NODE'S SUBCLUSTER CENTERS:
            MIN_DISTANCE = float('inf') # THE MINIMUM DISTANCE IS INITIALIZED TO INFINITY.
            ...
            for CHILD in NODE.CHILDREN: # FOR EACH CHILD IN THE NODE'S CHILDREN:
                DISTANCE = np.linalg.norm(SUBCLUSTER_CENTER - CHILD.PROTOTYPE) # THE DISTANCE BETWEEN THE SUBCLUSTER CENTER AND THE CHILD'S PROTOTYPE IS COMPUTED.
                if DISTANCE < MIN_DISTANCE: # IF THE DISTANCE IS LESS THAN THE MINIMUM DISTANCE:
                    MIN_DISTANCE = DISTANCE # THE MINIMUM DISTANCE IS SET TO THE DISTANCE.
                    CLOSEST_CHILD = CHILD # THE CLOSEST CHILD IS SET TO THE CHILD.
            if CLOSEST_CHILD is None or MIN_DISTANCE > CHILD_THRESHOLD: # IF THE CLOSEST CHILD IS NONE OR THE MINIMUM DISTANCE IS GREATER THAN THE CHILD THRESHOLD:
                NEW_NODE = BIRCH_NODE() # A NEW NODE IS CREATED.
                NEW_NODE.UPDATE_SUBCLUSTER_CENTERS(SUBCLUSTER_CENTER) # THE NEW NODE'S SUBCLUSTER CENTERS ARE UPDATED.
                NODE.ADD_CHILD(NEW_NODE) # THE NEW NODE IS ADDED AS A CHILD OF THE NODE.
            else: # IF THE CLOSEST CHILD IS NOT NONE AND THE MINIMUM DISTANCE IS NOT GREATER THAN THE CHILD THRESHOLD:
                CLOSEST_CHILD.UPDATE_SUBCLUSTER_CENTERS(SUBCLUSTER_CENTER) # THE CLOSEST CHILD'S SUBCLUSTER CENTERS ARE UPDATED.
        for CHILD in NODE.CHILDREN: # FOR EACH CHILD IN THE NODE'S CHILDREN:
        ...
        if NODE.IS_FULL(): # IF THE NODE IS FULL:
            ...
            NEW_NODE.CHILDREN = NODE.CHILDREN # THE NEW NODE'S CHILDREN ARE SET TO THE NODE'S CHILDREN.
            NODE.CHILDREN = [NEW_NODE] # THE NODE'S CHILDREN ARE SET TO THE NEW NODE.
    ...
    def __FIND_CLOSEST_CLUSTER__(POINT, NODE):
        ...
        if NODE.IS_LEAF: # IF THE NODE IS A LEAF NODE:
            MIN_DISTANCE = float('inf') # THE MINIMUM DISTANCE IS INITIALIZED TO INFINITY.
            CLOSEST_CLUSTER = None # THE CLOSEST CLUSTER IS INITIALIZED TO NONE.
            for SUBCLUSTER_CENTER in NODE.SUBCLUSTER_CENTERS: # FOR EACH SUBCLUSTER CENTER IN THE NODE'S SUBCLUSTER CENTERS:
                DISTANCE = np.linalg.norm(POINT - SUBCLUSTER_CENTER) # THE DISTANCE BETWEEN THE DATA POINT AND THE SUBCLUSTER CENTER IS CALCULATED.
                if DISTANCE < MIN_DISTANCE: # IF THE DISTANCE IS LESS THAN THE MINIMUM DISTANCE:
                    MIN_DISTANCE = DISTANCE # THE MINIMUM DISTANCE IS UPDATED.
                    CLOSEST_CLUSTER = SUBCLUSTER_CENTER # THE CLOSEST CLUSTER IS UPDATED.
            return CLOSEST_CLUSTER # THE CLOSEST CLUSTER IS RETURNED.
        MIN_DISTANCE = float('inf') # THE MINIMUM DISTANCE IS INITIALIZED TO INFINITY.
        CLOSEST_CHILD = None # THE CLOSEST CHILD IS INITIALIZED TO NONE.
        for CHILD in NODE.CHILDREN: # FOR EACH CHILD IN THE NODE'S CHILDREN:
            DISTANCE = np.linalg.norm(POINT - CHILD.PROTOTYPE) # THE DISTANCE BETWEEN THE DATA POINT AND THE CHILD'S PROTOTYPE IS CALCULATED.
            if DISTANCE < MIN_DISTANCE: # IF THE DISTANCE IS LESS THAN THE MINIMUM DISTANCE:
                MIN_DISTANCE = DISTANCE # THE MINIMUM DISTANCE IS UPDATED.
                CLOSEST_CHILD = CHILD # THE CLOSEST CHILD IS UPDATED.
        return __FIND_CLOSEST_CLUSTER__(POINT, CLOSEST_CHILD) # THE FUNCTION IS RECURSIVELY CALLED ON THE CLOSEST CHILD.
    
    ROOT = BIRCH_NODE() # A ROOT NODE IS CREATED.
    for POINT in X: # FOR EACH DATA POINT:
        __INSERT__(POINT, ROOT) # THE DATA POINT IS INSERTED INTO THE ROOT NODE.
    LABELS = np.zeros(len(X)) # THE LABELS ARE INITIALIZED TO ZERO.
    for i, POINT in enumerate(X): # FOR EACH DATA POINT:
        LABELS[i] = __FIND_CLOSEST_CLUSTER__(POINT, ROOT) # THE CLOSEST CLUSTER TO THE DATA POINT IS FOUND.
