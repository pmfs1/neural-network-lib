...
def K_MEANS_CLUSTERING(X, K=3, MAX_ITERATIONS=100):
    ...
    def __CREATE_CLUSTERS__(X, CENTROIDS):
        ...
        CLUSTERS = [[] for _ in range(len(CENTROIDS))] # INITIALIZE AN EMPTY LIST OF LISTS
        for IDX, SAMPLE in enumerate(X): # ITERATE OVER EACH DATA POINT
            CLOSEST_CENTROID_IDX = __CLOSEST_CENTROID__(SAMPLE, CENTROIDS) # FIND THE INDEX OF THE CLOSEST CENTROID
            CLUSTERS[CLOSEST_CENTROID_IDX].append(IDX) # APPEND THE INDEX OF THE CURRENT DATA POINT TO THE CORRESPONDING CLUSTER
    ...
    def __CLOSEST_CENTROID__(SAMPLE, CENTROIDS):
        ...
        DISTANCES = [__EUCLIDEAN_DISTANCE__(SAMPLE, CENTROID) for CENTROID in CENTROIDS] # CALCULATE THE EUCLIDEAN DISTANCES BETWEEN THE SAMPLE AND ALL CENTROIDS
        CLOSEST_IDX = np.argmin(DISTANCES) # FIND THE INDEX OF THE CENTROID WITH THE SMALLEST DISTANCE
    ...
    def __GET_CENTROIDS__(CLUSTERS):
        ...
        CENTROIDS = np.zeros((len(CLUSTERS), len(CLUSTERS[0][0]))) # INITIALIZE AN ARRAY OF ZEROS WITH DIMENSIONS (LEN(CLUSTERS), LEN(CLUSTERS[0][0]))
        for IDX, CLUSTER in enumerate(CLUSTERS): # ITERATE OVER EACH CLUSTER IN THE LIST OF CLUSTERS
            CLUSTER_MEAN = np.mean(CLUSTER, axis=0) # CALCULATE THE MEAN OF THE DATA POINTS IN THE CLUSTER ALONG THE FIRST AXIS
            CENTROIDS[IDX] = CLUSTER_MEAN # ASSIGN THE CALCULATED CENTROID AS THE VALUE FOR THE CORRESPONDING CLUSTER
    ...
    def __IS_CONVERGED__(OLD_CENTROIDS, NEW_CENTROIDS):
        ...
        DISTANCES = [__EUCLIDEAN_DISTANCE__(OLD_CENTROIDS[IDX], NEW_CENTROIDS[IDX]) for IDX in range(len(OLD_CENTROIDS))] # CALCULATE THE DISTANCE BETWEEN THE OLD AND NEW CENTROIDS FOR EACH CLUSTER
        return sum(DISTANCES) == 0 # RETURN TRUE IF THE SUM OF ALL DISTANCES IS EQUAL TO 0, INDICATING CONVERGENCE. OTHERWISE, RETURN FALSE.
    ...
    def __GET_CLUSTER_LABELS__(CLUSTERS):
        ...
        LABELS = np.empty(sum(len(_C) for _C in CLUSTERS)) # INITIALIZE AN EMPTY NUMPY ARRAY TO STORE THE CLUSTER LABELS FOR ALL DATA SAMPLES      
        for IDX, CLUSTER in enumerate(CLUSTERS): # ITERATE OVER EACH CLUSTER IN THE LIST OF CLUSTERS
            for SAMPLE_IDX in CLUSTER: # ITERATE OVER EACH SAMPLE INDEX IN THE CURRENT CLUSTER
                LABELS[SAMPLE_IDX] = IDX # ASSIGN THE CLUSTER INDEX AS THE CLUSTER LABEL FOR THAT SAMPLE IN THE `LABELS` ARRAY
    ...
    CENTROIDS = X[np.random.choice(N_SAMPLES, K, replace=False)] # RANDOMLY SELECT `K` SAMPLES FROM THE DATASET AS THE INITIAL CENTROIDS
    CLUSTERS = [[] for _ in range(K)] # INITIALIZE THE LIST OF CLUSTERS AS A LIST OF EMPTY LISTS
    for _ in range(MAX_ITERATIONS): # ITERATE OVER THE MAXIMUM NUMBER OF ITERATIONS
        NEW_CLUSTERS = __CREATE_CLUSTERS__(X, CENTROIDS) # CREATE NEW CLUSTERS BY ASSIGNING EACH DATA SAMPLE TO ITS CLOSEST CENTROID
        NEW_CENTROIDS = __GET_CENTROIDS__(NEW_CLUSTERS) # GET THE NEW CENTROIDS BY CALCULATING THE MEAN OF EACH CLUSTER
        if __IS_CONVERGED__(CENTROIDS, NEW_CENTROIDS): # CHECK IF THE CENTROIDS HAVE CONVERGED
    ...
    LABELS = __GET_CLUSTER_LABELS__(CLUSTERS) # GET THE CLUSTER LABELS FOR EACH DATA SAMPLE
...
def BISECTING_K_MEANS_CLUSTERING(X, K=3, MAX_ITERATIONS=100):
    ...
    while len(CLUSTERS) < K: # CONTINUE UNTIL THE DESIRED NUMBER OF CLUSTERS IS REACHED
        ...
        for CLUSTER in CLUSTERS: # FOR EACH CLUSTER
            ...
            SSE = np.sum([__EUCLIDEAN_DISTANCE__(POINT, CENTROID) ** 2 for POINT in CLUSTER]) # CALCULATE THE SSE
            SSE_VALUES.append(SSE) # APPEND THE SSE VALUE TO THE LIST
        CLUSTER_TO_SPLIT = CLUSTERS[np.argmax(SSE_VALUES)] # SELECT THE CLUSTER WITH THE HIGHEST SSE VALUE
        CLUSTERS.remove(CLUSTER_TO_SPLIT) # REMOVE THE SELECTED CLUSTER FROM THE LIST
        SPLITTED_CLUSTERS = K_MEANS_CLUSTERING(CLUSTER_TO_SPLIT, 2, MAX_ITERATIONS) # SPLIT THE SELECTED CLUSTER INTO TWO NEW CLUSTERS
        CLUSTERS.extend(SPLITTED_CLUSTERS) # ADD THE RESULTING CLUSTERS TO THE LIST
