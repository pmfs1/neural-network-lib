...
def GAUSSIAN_MIXTURES(X, K=2, MAX_ITERATIONS=100, TOLERANCE=1e-4):
    ...
    def __ESTIMATE_RESPONSIBILITIES__(X, WEIGHTS, MEANS, COVARIANCES):
        ...
        K = WEIGHTS.shape[0] # NUMBER OF COMPONENTS
        WEIGHTED_DENSITIES = np.zeros((N_SAMPLES, K)) # INITIALIZE WEIGHTED DENSITIES
        for i in range(K): # ITERATE OVER COMPONENTS
            WEIGHTED_DENSITIES[:, i] = WEIGHTS[i] * __MULTIVARIATE_NORMAL__(X, MEANS[i], COVARIANCES[i]) # COMPUTE WEIGHTED DENSITIES
        RESPONSIBILITY_MATRIX = WEIGHTED_DENSITIES / np.sum(WEIGHTED_DENSITIES, axis=1, keepdims=True) # COMPUTE RESPONSIBILITY MATRIX
        return RESPONSIBILITY_MATRIX # RETURN RESPONSIBILITY MATRIX
    ...
    def __ESTIMATE_PARAMETERS__(X, RESPONSIBILITY_MATRIX):
        ...
        N_SAMPLES = X.shape[0] # NUMBER OF SAMPLES
        K = RESPONSIBILITY_MATRIX.shape[1] # NUMBER OF COMPONENTS
        RESPONSIBILITY_MATRIX_SUM = np.sum(RESPONSIBILITY_MATRIX, axis=0) # SUM OF RESPONSIBILITIES ACROSS ALL SAMPLES FOR EACH COMPONENT
        WEIGHTS = RESPONSIBILITY_MATRIX_SUM / N_SAMPLES # UPDATE WEIGHTS
        MEANS = np.dot(RESPONSIBILITY_MATRIX.T, X) / RESPONSIBILITY_MATRIX_SUM[:, np.newaxis] # UPDATE MEANS
        COVARIANCES = [] # INITIALIZE LIST OF COVARIANCES
        for i in range(K): # ITERATE OVER EACH COMPONENT
            DIFFERENCE = X - MEANS[i] # DIFFERENCE BETWEEN DATA SAMPLES AND MEANS
            WEIGHTED_DIFFERENCE = RESPONSIBILITY_MATRIX[:, i][:, np.newaxis] * DIFFERENCE # WEIGHTED DIFFERENCES
            COVARIANCES.append(np.dot(WEIGHTED_DIFFERENCE.T, DIFFERENCE) / RESPONSIBILITY_MATRIX_SUM[i]) # UPDATE COVARIANCE MATRICES
        COVARIANCES = np.array(COVARIANCES) # CONVERT LIST OF COVARIANCES TO NUMPY ARRAY
        return WEIGHTS, MEANS, COVARIANCES # RETURN UPDATED WEIGHTS, MEANS, AND COVARIANCES
    ...
    def __MULTIVARIATE_NORMAL__(X, MEAN, COVARIANCE_MATRIX):
        ...
        DETERMINANT = np.linalg.det(COVARIANCE_MATRIX) # DETERMINANT OF COVARIANCE MATRIX
        NORMALIZATION_CONSTANT = 1.0 / ((2 * np.pi) ** (K / 2) * np.sqrt(DETERMINANT)) # NORMALIZATION CONSTANT
        INVERSE_OF_COVARIANCE_MATRIX = np.linalg.inv(COVARIANCE_MATRIX) # INVERSE OF COVARIANCE MATRIX
        EXPONENT = -0.5 * np.sum(np.dot((X - MEAN), INVERSE_OF_COVARIANCE_MATRIX) * (X - MEAN), axis=1) # EXPONENT OF MULTIVARIATE NORMAL DISTRIBUTION
        return NORMALIZATION_CONSTANT * np.exp(EXPONENT) # RETURN PROBABILITY DENSITY VALUES
    ...
    def __LOG_LIKELIHOOD__(X, WEIGHTS, MEANS, COVARIANCES):
        ...
        LOG_LIKELIHOODS = np.log(np.sum([WEIGHTS[i] * __MULTIVARIATE_NORMAL__(X, MEANS[i], COVARIANCES[i]) for i in range(WEIGHTS.shape[0])], axis=0)) # DETERMINE THE LOG-LIKELIHOOD OF THE GMM BY ITERATING OVER THE GAUSSIAN COMPONENTS, COMPUTING THE PROBABILITY DENSITY VALUES, WEIGHTING THEM BY THE COMPONENT WEIGHTS, AND CALCULATING THE LOG-LIKELIHOOD BASED ON THE SUM OF THESE WEIGHTED PROBABILITY DENSITY VALUES
        return np.sum(LOG_LIKELIHOODS) # RETURN THE LOG-LIKELIHOOD

    N_SAMPLES, N_FEATURES = X.shape # EXTRACT THE NUMBER OF SAMPLES AND FEATURES FROM THE SHAPE OF THE INPUT DATA
    WEIGHTS = np.ones(K) / K # INITIALIZE THE COMPONENT WEIGHTS TO BE UNIFORM
    MEANS = X[np.random.choice(N_SAMPLES, K, replace=False)] # INITIALIZE THE COMPONENT MEANS TO BE RANDOM SAMPLES FROM THE INPUT DATA
    COVARIANCES = np.array([np.eye(N_FEATURES)] * K) # INITIALIZE THE COMPONENT COVARIANCE MATRICES TO BE IDENTITY MATRICES
    PREVIOUS_LOG_LIKELIHOOD = 0.0 # INITIALIZE THE PREVIOUS LOG-LIKELIHOOD TO ZERO
    for _ in range(MAX_ITERATIONS): # ITERATE OVER THE MAXIMUM NUMBER OF ITERATIONS
        RESPONSIBILITY_MATRIX = __ESTIMATE_RESPONSIBILITIES__(X, WEIGHTS, MEANS, COVARIANCES) # ESTIMATE THE RESPONSIBILITY MATRIX
        WEIGHTS, MEANS, COVARIANCES = __ESTIMATE_PARAMETERS__(X, RESPONSIBILITY_MATRIX) # ESTIMATE THE PARAMETERS
        LOG_LIKELIHOOD = __LOG_LIKELIHOOD__(X, WEIGHTS, MEANS, COVARIANCES) # ESTIMATE THE LOG-LIKELIHOOD
        if np.abs(LOG_LIKELIHOOD - PREVIOUS_LOG_LIKELIHOOD) < TOLERANCE: # IF THE DIFFERENCE BETWEEN THE LOG-LIKELIHOOD AND THE PREVIOUS LOG-LIKELIHOOD IS LESS THAN THE TOLERANCE
            break # BREAK
        PREVIOUS_LOG_LIKELIHOOD = LOG_LIKELIHOOD # UPDATE THE PREVIOUS LOG-LIKELIHOOD
    RESPONSIBILITY_MATRIX = __ESTIMATE_RESPONSIBILITIES__(X, WEIGHTS, MEANS, COVARIANCES) # ESTIMATE THE RESPONSIBILITY MATRIX
    LABELS = np.argmax(RESPONSIBILITY_MATRIX, axis=1) # DETERMINE THE LABELS BY TAKING THE ARGUMENTS OF THE MAXIMUM VALUES IN THE RESPONSIBILITY MATRIX
