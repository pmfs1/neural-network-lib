...
def WARD_HIERARCHICAL_CLUSTERING(X):
    ...
    def __CALCULATE_MERGE_DISTANCE__(CLUSTER_A, CLUSTER_B, DISTANCES):
        ...
        TOTAL_DISTANCE = 0.0 # INITIALIZE THE TOTAL DISTANCE BETWEEN THE CLUSTERS
        for i in CLUSTER_A: # ITERATE OVER ALL PAIRS OF DATA POINTS FROM `CLUSTER_A` AND `CLUSTER_B`
            for j in CLUSTER_B: # RETRIEVE THE PAIRWISE DISTANCE BETWEEN EACH PAIR OF DATA POINTS FROM THE `DISTANCES` MATRIX
                TOTAL_DISTANCE += DISTANCES[i, j] # ACCUMULATE THE DISTANCE VALUES TO THE TOTAL DISTANCE
        return TOTAL_DISTANCE / (len(CLUSTER_A) * len(CLUSTER_B)) # RETURN THE AVERAGE DISTANCE BETWEEN THE CLUSTERS

    N_SAMPLES = len(X) # GET THE NUMBER OF DATA POINTS
    DISTANCES = np.zeros((N_SAMPLES, N_SAMPLES)) # INITIALIZE A MATRIX TO STORE PAIRWISE DISTANCES BETWEEN DATA POINTS
    for i in range(N_SAMPLES): # ITERATE OVER ALL PAIRS OF DATA POINTS
        for j in range(i + 1, N_SAMPLES): # CALCULATE THE EUCLIDEAN DISTANCE BETWEEN EACH PAIR OF DATA POINTS
            DISTANCES[i, j] = DISTANCES[j, i] = __EUCLIDEAN_DISTANCE__(X[i], X[j]) # STORE THE DISTANCE IN THE `DISTANCES` MATRIX
    CLUSTERS = [[i] for i in range(N_SAMPLES)] # INITIALIZE A LIST OF CLUSTERS, WHERE EACH CLUSTER CONTAINS A SINGLE DATA POINT
    while len(CLUSTERS) > 1: # ITERATE UNTIL THERE IS ONLY ONE CLUSTER LEFT
        MIN_DISTANCE = np.inf # INITIALIZE THE MINIMUM DISTANCE BETWEEN TWO CLUSTERS
        MERGE_I, MERGE_J = -1, -1 # INITIALIZE THE INDICES OF THE CLOSEST CLUSTERS
        for i, _ in enumerate(CLUSTERS): # ITERATE OVER ALL PAIRS OF CLUSTERS
            for j, _ in enumerate(CLUSTERS, start=i + 1): # CALCULATE THE MERGE DISTANCE BETWEEN EACH PAIR OF CLUSTERS
                DISTANCE = __CALCULATE_MERGE_DISTANCE__(CLUSTERS[i], CLUSTERS[j], DISTANCES) # UPDATE THE CLOSEST CLUSTERS IF THE MERGE DISTANCE IS SMALLER
                if DISTANCE < MIN_DISTANCE: # UPDATE THE CLOSEST CLUSTERS IF THE MERGE DISTANCE IS SMALLER
                    MIN_DISTANCE = DISTANCE # UPDATE THE MINIMUM DISTANCE BETWEEN TWO CLUSTERS
                    MERGE_I, MERGE_J = i, j # UPDATE THE INDICES OF THE CLOSEST CLUSTERS
        CLUSTERS[MERGE_I].extend(CLUSTERS[MERGE_J]) # MERGE THE CLOSEST CLUSTERS
        del CLUSTERS[MERGE_J] # REMOVE THE SECOND CLOSEST CLUSTER
    return CLUSTERS # RETURN THE FINAL CLUSTERING
