...
def WARD_HIERARCHICAL_CLUSTERING(X: np.ndarray) -> list:
    ...
    def __CALCULATE_MERGE_DISTANCE__(CLUSTER_A: list, CLUSTER_B: list, DISTANCES: np.ndarray) -> float|np.ndarray:
        ...
        for i in CLUSTER_A:  # ITERATE OVER ALL PAIRS OF DATA POINTS FROM `CLUSTER_A` AND `CLUSTER_B`
            for j in CLUSTER_B:  # RETRIEVE THE PAIRWISE DISTANCE BETWEEN EACH PAIR OF DATA POINTS FROM THE `DISTANCES` MATRIX
                # ACCUMULATE THE DISTANCE VALUES TO THE TOTAL DISTANCE
                TOTAL_DISTANCE += DISTANCES[i, j]
        # RETURN THE AVERAGE DISTANCE BETWEEN THE CLUSTERS
        return TOTAL_DISTANCE / (len(CLUSTER_A) * len(CLUSTER_B))

    N_SAMPLES = len(X)  # GET THE NUMBER OF DATA POINTS
    # INITIALIZE A MATRIX TO STORE PAIRWISE DISTANCES BETWEEN DATA POINTS
    ...
    for i in range(N_SAMPLES):  # ITERATE OVER ALL PAIRS OF DATA POINTS
        ...
        for j in range(i + 1, N_SAMPLES):
            DISTANCES[i, j] = DISTANCES[j, i] = __EUCLIDEAN_DISTANCE__(
                X[i], X[j])  # STORE THE DISTANCE IN THE `DISTANCES` MATRIX
    # INITIALIZE A LIST OF CLUSTERS, WHERE EACH CLUSTER CONTAINS A SINGLE DATA POINT
    ...
    while len(CLUSTERS) > 1:  # ITERATE UNTIL THERE IS ONLY ONE CLUSTER LEFT
        ...
        for i, _ in enumerate(CLUSTERS):  # ITERATE OVER ALL PAIRS OF CLUSTERS
            # CALCULATE THE MERGE DISTANCE BETWEEN EACH PAIR OF CLUSTERS
            for j, _ in enumerate(CLUSTERS, start=i + 1):
                # UPDATE THE CLOSEST CLUSTERS IF THE MERGE DISTANCE IS SMALLER
                DISTANCE = __CALCULATE_MERGE_DISTANCE__(
