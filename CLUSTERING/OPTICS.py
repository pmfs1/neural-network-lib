...
#         - `EPSILON` (OPTIONAL): THE MAXIMUM DISTANCE BETWEEN TWO SAMPLES FOR THEM TO BE CONSIDERED NEIGHBOURS. IF NOT PROVIDED, IT IS CALCULATED BASED ON THE AVERAGE DISTANCE BETWEEN POINTS IN THE DATASET.
...
def OPTICS(X, MIN_SAMPLES, EPSILON=None):
    ...
    def __CALCULATE_REACHABILITY_DISTANCES__(DISTANCES, CORE_DISTANCES, EPSILON):
        ...
        REACHABILITY_DISTANCES = np.full(N_POINTS, np.inf) # THE `REACHABILITY_DISTANCES` ARRAY IS INITIALIZED WITH THE SIZE OF `N_POINTS` AND FILLED WITH INFINITY VALUES.
        for POINT in range(N_POINTS): # THE FUNCTION ITERATES OVER EACH POINT IN THE DATASET USING A LOOP.
            if CORE_DISTANCES[POINT] < np.inf: # FOR EACH POINT, IT CHECKS IF THE CORE DISTANCE OF THE POINT IS LESS THAN INFINITY. IF IT IS, IT MEANS THE POINT IS A CORE POINT.
                NEIGHBOURHOOD = np.where(DISTANCES[POINT] <= EPSILON)[0] # THE NEIGHBOURHOOD OF THE CURRENT POINT IS IDENTIFIED BY FINDING THE INDICES OF THE POINTS WHOSE DISTANCE TO THE CURRENT POINT IS LESS THAN OR EQUAL TO `EPSILON`.
                REACHABILITY_DISTANCES[POINT] = np.max(np.maximum(CORE_DISTANCES[POINT], DISTANCES[POINT][NEIGHBOURHOOD])) # THE REACHABILITY DISTANCE FOR THE CURRENT POINT IS CALCULATED AS THE MAXIMUM VALUE BETWEEN THE CORE DISTANCE OF THE POINT ITSELF AND THE MAXIMUM DISTANCE TO ITS NEIGHBOURHOOD.
        return REACHABILITY_DISTANCES # THE `__CALCULATE_REACHABILITY_DISTANCES__` FUNCTION RETURNS THE ARRAY OF REACHABILITY DISTANCES.
    ...
    def __EXTRACT_CLUSTERS__(DISTANCES, REACHABILITY_DISTANCES, EPSILON, CORE_DISTANCES):
        ...
        ORDERED_POINTS = np.argsort(REACHABILITY_DISTANCES) # ORDERED POINTS BY REACHABILITY DISTANCE
        CLUSTERS = [] # LIST OF CLUSTERS: EACH CLUSTER IS A LIST OF POINTS
        for POINT in ORDERED_POINTS: # ITERATE OVER THE ORDERED POINTS
            if REACHABILITY_DISTANCES[POINT] > EPSILON: # IF REACHABILITY DISTANCE > EPSILON, POINT IS NOT PART OF A CLUSTER
                if CORE_DISTANCES[POINT] <= EPSILON: # IF CORE DISTANCE <= EPSILON, POINT IS A POTENTIAL SEED OF A NEW CLUSTER
                    CURRENT_CLUSTER = [POINT] # START A NEW CLUSTER
                    IDX = np.where(ORDERED_POINTS == POINT)[0][0] + 1 # FIND THE INDEX OF THE CURRENT POINT IN THE ORDERED POINTS LIST
                    while IDX < N_POINTS and REACHABILITY_DISTANCES[ORDERED_POINTS[IDX]] <= EPSILON: # WHILE REACHABILITY DISTANCE <= EPSILON
                        CURRENT_CLUSTER.append(ORDERED_POINTS[IDX]) # ADD THE NEXT ORDERED POINT TO THE CURRENT CLUSTER
                        IDX += 1 # INCREMENT IDX
                    CLUSTERS.append(CURRENT_CLUSTER) # APPEND THE CURRENT CLUSTER TO THE LIST OF CLUSTERS
        return CLUSTERS # RETURN THE LIST OF CLUSTERS

    DISTANCES = __CALCULATE_DISTANCES__(X) # CALCULATE THE PAIRWISE DISTANCES BETWEEN ALL POINTS IN THE DATASET
    if EPSILON is None: # IF EPSILON IS NOT PROVIDED
        AVG_DISTANCE = np.mean(DISTANCES) # CALCULATE THE AVERAGE DISTANCE BETWEEN ALL POINTS
        EPSILON = AVG_DISTANCE * 0.5 # SET EPSILON TO HALF THE AVERAGE DISTANCE
    CORE_DISTANCES = __CALCULATE_CORE_DISTANCES__(DISTANCES, MIN_SAMPLES) # CALCULATE THE CORE DISTANCES FOR ALL POINTS
    REACHABILITY_DISTANCES = __CALCULATE_REACHABILITY_DISTANCES__(DISTANCES, CORE_DISTANCES, EPSILON) # CALCULATE THE REACHABILITY DISTANCES FOR ALL POINTS
    CLUSTERS = __EXTRACT_CLUSTERS__(DISTANCES, REACHABILITY_DISTANCES, EPSILON, CORE_DISTANCES) # EXTRACT CLUSTERS FROM THE REACHABILITY DISTANCES
    return CLUSTERS # RETURN THE LIST OF CLUSTERS
...
def PDIST(X):
    ...
    PAIRWISE_DISTANCES = np.zeros((N * (N - 1)) // 2) # ARRAY TO STORE THE PAIRWISE DISTANCES
    ...
    for i in range(N - 1): # ITERATE OVER THE INDICES OF THE POINTS
        for j in range(i + 1, N): # ITERATE OVER THE INDICES OF THE POINTS
            DISTANCE = np.sqrt(np.sum((X[i] - X[j]) ** 2)) # CALCULATE THE EUCLIDEAN DISTANCE BETWEEN THE POINTS
            PAIRWISE_DISTANCES[IDX] = DISTANCE # STORE THE DISTANCE IN THE ARRAY
...
def SQUAREFORM(PAIRWISE_DISTANCES):
    ...
    N = int(np.sqrt(2 * len(PAIRWISE_DISTANCES)) + 0.5) # NUMBER OF POINTS IN THE DATASET
    SQUARE_DISTANCES = np.zeros((N, N)) # ARRAY TO STORE THE SQUARE DISTANCES
    IDX = 0 # INDEX TO KEEP TRACK OF THE CURRENT POSITION IN THE ARRAY
    for i in range(N - 1): # ITERATE OVER THE INDICES OF THE POINTS
        for j in range(i + 1, N): # ITERATE OVER THE INDICES OF THE POINTS
            SQUARE_DISTANCES[i, j] = PAIRWISE_DISTANCES[IDX] # ASSIGN THE DISTANCE TO THE CORRESPONDING POSITION IN THE MATRIX
            SQUARE_DISTANCES[j, i] = PAIRWISE_DISTANCES[IDX] # ASSIGN THE DISTANCE TO THE CORRESPONDING POSITION IN THE MATRIX
            IDX += 1 # INCREMENT THE INDEX
