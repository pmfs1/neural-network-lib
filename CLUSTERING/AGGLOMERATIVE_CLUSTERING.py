...
def AGGLOMERATIVE_CLUSTERING(X, K):
    ...
    N_SAMPLES_IN_CLUSTERS = np.ones(N_SAMPLES) # ARRAY OF NUMBER OF SAMPLES IN EACH CLUSTER
    ...
    def __MERGE_CLUSTERS__(MERGE_IDXS):
        ...
        for i, _ in enumerate(LABELS): # ITERATE THROUGH ALL SAMPLES
            if LABELS[i] == CLUSTER_2: # IF SAMPLE BELONGS TO SECOND CLUSTER
                ...
                N_SAMPLES_IN_CLUSTERS[CLUSTER_1] += 1 # INCREMENT SAMPLE COUNT IN FIRST CLUSTER
        N_SAMPLES_IN_CLUSTERS[CLUSTER_2] = 0 # SET SAMPLE COUNT IN SECOND CLUSTER TO 0

    while len(np.unique(LABELS)) > K: # WHILE NUMBER OF CLUSTERS IS GREATER THAN DESIRED NUMBER OF CLUSTERS
        ...
        for i in range(N_SAMPLES - 1): # ITERATE THROUGH ALL SAMPLES
            for j in range(i + 1, N_SAMPLES): # ITERATE THROUGH ALL SAMPLES AFTER CURRENT SAMPLE
                if LABELS[i] != LABELS[j]: # IF SAMPLES BELONG TO DIFFERENT CLUSTERS
                    DISTANCE = __EUCLIDEAN_DISTANCE__(X[i], X[j]) # CALCULATE DISTANCE BETWEEN SAMPLES
                    if DISTANCE < MIN_DISTANCE: # IF DISTANCE IS LESS THAN MINIMUM DISTANCE
                        MIN_DISTANCE = DISTANCE # UPDATE MINIMUM DISTANCE
                        MERGE_IDXS = (i, j) # UPDATE INDICES OF SAMPLES TO BE MERGED
