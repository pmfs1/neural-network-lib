...
def HDBSCAN(X, MIN_SAMPLES=5):
    ...
    def __COMPUTE_CORE_DISTANCES__(X):
        ...
        DISTANCES = PAIRWISE_DISTANCES(X) # THE `PAIRWISE_DISTANCES` FUNCTION CALCULATES THE PAIRWISE DISTANCES BETWEEN ALL SAMPLES IN THE DATASET.
        SORTED_DISTANCES = np.sort(DISTANCES, axis=0) # THE `SORTED_DISTANCES` ARRAY IS CREATED BY SORTING THE `DISTANCES` MATRIX ALONG THE ROWS (AXIS 0). SORTING THE DISTANCES ALLOWS US TO ACCESS THE `MIN_SAMPLES`-TH NEAREST NEIGHBOR DISTANCE FOR EACH SAMPLE.
        CORE_DISTANCES = SORTED_DISTANCES[MIN_SAMPLES - 1] # THE `MIN_SAMPLES - 1` INDEX IS USED TO EXTRACT THE `MIN_SAMPLES`-TH NEAREST NEIGHBOR DISTANCES FROM THE SORTED DISTANCES. THIS VALUE REPRESENTS THE CORE DISTANCE FOR EACH SAMPLE.
        return CORE_DISTANCES # THE CORE DISTANCES ARE STORED IN THE `CORE_DISTANCES` ARRAY.
    ...
    def __COMPUTE_NEIGHBOURS__(X, CORE_DISTANCES):
        ...
        NEIGHBOURS = np.argwhere(PAIRWISE_DISTANCES(X) <= CORE_DISTANCES)[:, 1] # THE `NEIGHBOURS` ARRAY IS CREATED BY FINDING THE INDICES OF SAMPLES WHOSE DISTANCES ARE LESS THAN OR EQUAL TO THE CORE DISTANCE OF THAT SAMPLE. THIS IS DONE USING THE `NP.ARGWHERE` FUNCTION, WHICH RETURNS THE INDICES WHERE A CERTAIN CONDITION IS SATISFIED.
        return NEIGHBOURS # THE `NEIGHBOURS` ARRAY, CONTAINING THE INDICES OF NEIGHBORS FOR EACH SAMPLE IN THE DATASET, IS RETURNED AS THE OUTPUT OF THE `__COMPUTE_NEIGHBOURS__` FUNCTION.
    ...
    def __CONDENSE_TREE__(X, NEIGHBOURS, CORE_DISTANCES):
        ...
        for IDX, _ in enumerate(X): # ITERATE OVER EACH SAMPLE IN THE DATASET
            NEIGHBOURS = NEIGHBOURS[NEIGHBOURS == IDX] # SELECT THE NEIGHBORS OF THE CURRENT SAMPLE
            if len(NEIGHBOURS) > 0: # IF THERE ARE NEIGHBORS FOR THE CURRENT SAMPLE
                MIN_DISTANCE_IDX = np.argmin(CORE_DISTANCES[NEIGHBOURS]) # FIND THE NEAREST NEIGHBOR BASED ON THEIR CORE DISTANCES
                NEAREST_NEIGHBOUR = NEIGHBOURS[MIN_DISTANCE_IDX] # GET THE INDEX OF THE NEAREST NEIGHBOR
                EDGE_LENGTH = CORE_DISTANCES[NEAREST_NEIGHBOUR] # GET THE LENGTH OF THE EDGE BETWEEN THE CURRENT SAMPLE AND ITS NEAREST NEIGHBOR
                CONDENSED_TREE[(IDX, NEAREST_NEIGHBOUR)] = EDGE_LENGTH # ADD THE EDGE BETWEEN THE CURRENT SAMPLE AND ITS NEAREST NEIGHBOR TO THE CONDENSED TREE
    ...
    def __EXTRACT_CLUSTERS__(CONDENSED_TREE, N_SAMPLES):
        ...
        CLUSTER_ID = 0 # INITIALIZE A VARIABLE TO KEEP TRACK OF THE CURRENT CLUSTER ID. THE ID STARTS AT 0 AND INCREMENTS AS NEW CLUSTERS ARE DISCOVERED.
        for EDGE, _ in sorted(CONDENSED_TREE.items(), key=lambda x: x[1]): # ITERATE OVER THE EDGES OF THE CONDENSED TREE. THE EDGES REPRESENT CONNECTIONS BETWEEN SAMPLES, AND THEY ARE SORTED BASED ON THEIR EDGE LENGTHS IN ASCENDING ORDER.
            CLUSTER_A, CLUSTER_B = EDGE # EXTRACT THE TWO SAMPLES CONNECTED BY THE EDGE.
            if LABELS[CLUSTER_A] == -1 and LABELS[CLUSTER_B] == -1: # IF BOTH SAMPLES HAVE UNASSIGNED LABELS, IT MEANS THAT THEY ARE NOT PART OF ANY CLUSTER YET. IN THIS CASE, ASSIGN THEM THE SAME CLUSTER ID AND INCREMENT THE CLUSTER ID BY 1.
                LABELS[CLUSTER_A] = CLUSTER_ID # ASSIGN THE CURRENT CLUSTER ID TO THE FIRST SAMPLE.
                LABELS[CLUSTER_B] = CLUSTER_ID # ASSIGN THE CURRENT CLUSTER ID TO THE SECOND SAMPLE.
                CLUSTER_ID += 1 # INCREMENT THE CLUSTER ID BY 1.
            elif LABELS[CLUSTER_A] == -1: # IF ONLY THE FIRST SAMPLE HAS AN UNASSIGNED LABEL, IT MEANS THAT IT IS NOT PART OF ANY CLUSTER YET. IN THIS CASE, ASSIGN IT THE SAME CLUSTER ID AS THE SECOND SAMPLE.
                LABELS[CLUSTER_A] = LABELS[CLUSTER_B] # ASSIGN THE LABEL OF THE SECOND SAMPLE TO THE FIRST SAMPLE.
            elif LABELS[CLUSTER_B] == -1: # IF ONLY THE SECOND SAMPLE HAS AN UNASSIGNED LABEL, IT MEANS THAT IT IS NOT PART OF ANY CLUSTER YET. IN THIS CASE, ASSIGN IT THE SAME CLUSTER ID AS THE FIRST SAMPLE.
                LABELS[CLUSTER_B] = LABELS[CLUSTER_A] # ASSIGN THE LABEL OF THE FIRST SAMPLE TO THE SECOND SAMPLE.
            elif LABELS[CLUSTER_A] != LABELS[CLUSTER_B]: # IF BOTH SAMPLES HAVE ASSIGNED LABELS, IT MEANS THAT THEY ARE PART OF DIFFERENT CLUSTERS. IN THIS CASE, MERGE THE TWO CLUSTERS BY ASSIGNING A COMMON LABEL TO ALL SAMPLES IN THE CONNECTED CLUSTERS.
                MERGE_LABEL = min(LABELS[CLUSTER_A], LABELS[CLUSTER_B]) # DETERMINE THE LABEL OF THE CLUSTER TO WHICH THE SAMPLES WILL BE MERGED. THE MERGE LABEL IS THE SMALLER OF THE TWO LABELS.
                LABELS[np.logical_or(LABELS == LABELS[CLUSTER_A], LABELS == LABELS[CLUSTER_B])] = MERGE_LABEL # ASSIGN THE MERGE LABEL TO ALL SAMPLES IN THE CONNECTED CLUSTERS.
        return __POST_PROCESS_LABELS__(LABELS) # RETURN THE CLUSTER LABELS AFTER APPLYING THE POST-PROCESSING STEP.
    ...
    def __POST_PROCESS_LABELS__(LABELS):
        UNIQUE_LABELS = np.unique(LABELS) # IDENTIFY THE UNIQUE CLUSTER LABELS PRESENT IN THE `LABELS` ARRAY.
        NEW_LABELS = -1 * np.ones_like(LABELS) # INITIALIZE AN ARRAY OF THE SAME SIZE AS `LABELS`, INITIALLY FILLED WITH -1.
        for IDX, LABEL in enumerate(UNIQUE_LABELS): # ITERATE OVER THE UNIQUE CLUSTER LABELS ALONG WITH THEIR CORRESPONDING INDICES.
            NEW_LABELS[LABELS == LABEL] = IDX # ASSIGN A NEW LABEL INDEX TO ALL SAMPLES IN `LABELS` THAT HAVE THE SAME LABEL.
    ...
    CORE_DISTANCES = __COMPUTE_CORE_DISTANCES__(X) # COMPUTE THE CORE DISTANCES FOR ALL SAMPLES IN `X`.
    NEIGHBOURS = __COMPUTE_NEIGHBOURS__(X, CORE_DISTANCES) # COMPUTE THE NEIGHBOURS FOR ALL SAMPLES IN `X`.
    CONDENSED_TREE = __CONDENSE_TREE__(X, NEIGHBOURS, CORE_DISTANCES) # CONSTRUCT THE CONDENSED TREE FOR THE SAMPLES IN `X`.
    LABELS = __EXTRACT_CLUSTERS__(CONDENSED_TREE, N_SAMPLES) # EXTRACT THE CLUSTERS FROM THE CONDENSED TREE.
...
def PAIRWISE_DISTANCES(X):
    ...
    N_SAMPLES = X.shape[0] # ASSIGN THE NUMBER OF SAMPLES IN `X` TO THE VARIABLE `N_SAMPLES`.
    DISTANCES = np.zeros((N_SAMPLES, N_SAMPLES)) # INITIALIZE AN EMPTY DISTANCE MATRIX OF SIZE `N_SAMPLES` BY `N_SAMPLES`.
    for i in range(N_SAMPLES): # ITERATE OVER THE SAMPLES IN `X`.
        for j in range(i + 1, N_SAMPLES): # ITERATE OVER THE SAMPLES IN `X` STARTING FROM THE CURRENT SAMPLE.
            DISTANCES[i, j] = DISTANCES[j, i] = np.linalg.norm(X[i] - X[j]) # CALCULATE THE EUCLIDEAN DISTANCE BETWEEN THE CURRENT SAMPLE AND THE `J`-TH SAMPLE.
