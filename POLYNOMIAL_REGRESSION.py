...
class POLYNOMIAL_REGRESSION:
    ...
    # LOSS FUNCTION [PRIVATE]: CALCULATES THE LOSS FUNCTION: MEAN SQUARED ERROR
    def __LOSS_FUNCTION__(TRUE_TARGET_VALUE, HYPOTHESIS):
        return np.mean((HYPOTHESIS - TRUE_TARGET_VALUE) ** 2) # CALCULATES THE LOSS FUNCTION: MEAN SQUARED ERROR
    
    # GRADIENTS [PRIVATE]: CALCULATES THE GRADIENTS OF THE LOSS FUNCTION W.R.T. THE PARAMETERS: WEIGHTS AND BIAS
    def __GRADIENTS__(X, Y, HYPOTHESIS):
    ...
    def __X_TRANSFORM__(X, DEGREES):
        T = X.copy() # COPY OF X
        for DEGREE in DEGREES: # FOR EACH DEGREE IN DEGREES
            X = np.append(X, T ** DEGREE, axis=1) # ADD X^(DEGREE) TO X
        return X # RETURNS X
    
    # R2_SCORE [STATIC]: CALCULATES THE R2 SCORE
    @staticmethod
    def R2_SCORE(Y, HYPOTHESIS):
        return 1 - (np.sum((np.array(HYPOTHESIS) - np.array(Y)) ** 2) / np.sum((np.array(Y) - np.mean(np.array(Y))) ** 2)) # CALCULATES THE R2 SCORE