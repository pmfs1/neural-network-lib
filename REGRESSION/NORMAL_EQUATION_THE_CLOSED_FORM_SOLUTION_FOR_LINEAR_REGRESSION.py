...
class NORMAL_EQUATION_THE_CLOSED_FORM_SOLUTION_FOR_LINEAR_REGRESSION:
    ...
    METHODS
    -------
    FIND_THETA(X, Y): RETURNS THE PARAMETERS, THETA, OF THE LINEAR REGRESSION MODEL: THETA IS A VECTOR OF WEIGHTS.
    ...
    def PLOT(self, X, Y):
        ...
        PREDICTIONS = self.PREDICT(X) # GETTING THE PREDICTIONS
        plt.figure(figsize=(8,6)) # DEFINING THE FIGURE SIZE
        plt.plot(X, Y, 'b.') # PLOTTING THE TRAINING DATA
        plt.plot(X, PREDICTIONS, 'c-') # PLOTTING THE PREDICTIONS
        plt.xlabel('X') # DEFINING THE X-LABEL
        plt.ylabel('Y') # DEFINING THE Y-LABEL
        plt.title('NORMAL EQUATION (THE CLOSED FORM SOLUTION) FOR LINEAR REGRESSION') # DEFINING THE TITLE
