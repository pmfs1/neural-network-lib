...
class LOCALLY_WEIGHTED_LINEAR_REGRESSION:
    ...
    def __WEIGHT_MATRIX__(self, POINT, X):
        ...
        WEIGHT_MATRIX = np.mat(np.eye(NUMBER_OF_SAMPLES)) # INITIALIZES THE WEIGHT MATRIX AS AN IDENTITY MATRIX
        for EXAMPLE_INDEX in range(NUMBER_OF_SAMPLES): # CALCULATES THE WEIGHTS FOR EACH TRAINING EXAMPLE
    ...
    # TRANSFORM(): RETURNS THE PREDICTIONS OF THE LOCALLY WEIGHTED LINEAR REGRESSION MODEL
    def TRANSFORM(self, X, Y, POINT):
        NUMBER_OF_SAMPLES = X.shape[0] # NUMBER OF TRAINING EXAMPLES
        X_ = np.append(X, np.ones(NUMBER_OF_SAMPLES).reshape(NUMBER_OF_SAMPLES, 1), axis=1) # ADDS A COLUMN OF ONES TO THE X MATRIX
