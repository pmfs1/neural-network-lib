...
class LOCALLY_WEIGHTED_LINEAR_REGRESSION:
    ...
    PLOT(X, Y, N_POINTS): PLOTS THE PREDICTIONS OF THE LOCALLY WEIGHTED LINEAR REGRESSION MODEL FOR THE INPUT DATA X AND THE REAL DATA Y. THE PLOT IS MADE WITH N_POINTS POINTS.
    """
    ...
    # PLOT(): PLOTS THE PREDICTIONS OF THE LOCALLY WEIGHTED LINEAR REGRESSION MODEL FOR THE INPUT DATA X AND THE REAL DATA Y. THE PLOT IS MADE WITH N_POINTS POINTS.
    def PLOT(self, X, Y, N_POINTS=100):
        """PLOTS THE PREDICTIONS OF THE LOCALLY WEIGHTED LINEAR REGRESSION MODEL FOR THE INPUT DATA X AND THE REAL DATA Y. THE PLOT IS MADE WITH N_POINTS POINTS.
        ...
        Y: NUMPY ARRAY SHAPE (NUMBER OF TRAINING EXAMPLES, 1)
            IT'S THE LABELS OF THE TRAINING DATA.
        N_POINTS: INTEGER (DEFAULT: 100)
            IT'S THE NUMBER OF POINTS THAT THE PLOT WILL HAVE.
        ...
        X_TEST = np.linspace(-3, 3, N_POINTS) # GENERATES N_POINTS POINTS
        PREDICTIONS = [] # INITIALIZES THE PREDICTIONS
        for POINT in X_TEST: # FOR EACH POINT
            _, PREDICTION = self.PREDICT(X, Y, POINT) # CALCULATES THE PREDICTION
            PREDICTIONS.append(PREDICTION) # APPENDS THE PREDICTION
        X_TEST = np.array(X_TEST).reshape(N_POINTS,1) # RESHAPES THE X_TEST
        PREDICTIONS = np.array(PREDICTIONS).reshape(N_POINTS,1) # RESHAPES THE PREDICTIONS
        plt.plot(X, Y, 'b.') # PLOTS THE REAL DATA
        plt.plot(X_TEST, PREDICTIONS, 'r.') # PLOTS THE PREDICTIONS
        plt.xlabel('X') # SETS THE X LABEL
        plt.ylabel('Y') # SETS THE Y LABEL
        plt.title('LOCALLY WEIGHTED LINEAR REGRESSION') # SETS THE TITLE
