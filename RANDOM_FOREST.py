...
class RANDOM_FOREST:
    ...
    # PREDICT(): METHOD THAT PREDICTS THE DATASET INTO A MATRIX OF PREDICTIONS
    def PREDICT(self, X):
        # CREATES A MATRIX OF PREDICTIONS
        TREE_PREDICTIONS = np.array([TREE.PREDICT(X) for TREE in self.TREES])
        # TRANSPOSES THE MATRIX OF PREDICTIONS
        TREE_PREDICTIONS = np.swapaxes(TREE_PREDICTIONS, 0, 1)
        Y_PREDICTIONS = [self.__MOST_COMMON_LABEL__(
            TREE_PREDICTION) for TREE_PREDICTION in TREE_PREDICTIONS]  # CREATES A LIST OF PREDICTIONS
        return np.array(Y_PREDICTIONS)  # RETURNS THE LIST OF PREDICTIONS
