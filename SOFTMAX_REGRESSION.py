...
# SOFTMAX_REGRESSION: IMPLEMENTS THE SOFTMAX REGRESSION MODEL. WHICH IS A GENERALIZATION OF LOGISTIC REGRESSION TO THE CASE WHERE WE WANT TO HANDLE MULTIPLE CLASSES.
...
class SOFTMAX_REGRESSION:
    ...
    def FIT(self, X, Y):
        ...
        for _ in range(self.EPOCHS):  # LOOPING THROUGH THE EPOCHS
            Z = X@self.WEIGHTS + self.BIAS  # CALCULATING THE PREDICTION/PREDICTION
            # CALCULATING THE SOFTMAX OF THE PREDICTION/PREDICTION
            PREDICTION = SOFTMAX(Z)
            # CONVERTING THE LABELS TO ONE HOT ENCODING FORMAT
            Y_HOT = self.ONE_HOT(Y)
            # CALCULATING THE GRADIENT OF THE LOSS W.R.T WEIGHTS
            W_GRADIENT = (1 / NUMBER_OF_SAMPLES) * \
                np.dot(X.T, (PREDICTION - Y_HOT))
            # CALCULATING THE GRADIENT OF THE LOSS W.R.T BIAS
            B_GRADIENT = (1 / NUMBER_OF_SAMPLES) * np.sum(PREDICTION - Y_HOT)
    ...
    def TRANSFORM(self, X):
        ...
        # CALCULATING THE SOFTMAX OF THE PREDICTION/PREDICTION
        PREDICTION = SOFTMAX(Z)
        return np.argmax(PREDICTION, axis=1)  # RETURNING THE PREDICTED LABELS
