...
class DECISION_TREE:
    ...
    def PREDICT(self, X):
        # RETURNS THE PREDICTED DATASET: [TRAVERSE_TREE(X[I], ROOT) FOR X[I] IN X]. IT'S THE OUTPUT OF THE __TRAVERSE_TREE__() FUNCTION.
    ...
    def __TRAVERSE_TREE__(self, X, CURRENT_NODE):
        if CURRENT_NODE.LEAF_VALUE is not None:  # IF CURRENT_NODE.LEAF_VALUE IS NOT NONE, THEN
            return CURRENT_NODE.LEAF_VALUE  # RETURNS CURRENT_NODE.LEAF_VALUE
        # FEATURE_VALUE: IT'S THE VALUE OF THE FEATURE WITH INDEX CURRENT_NODE.FEATURE_INDEX IN THE INPUT X.
        FEATURE_VALUE = X[CURRENT_NODE.FEATURE_INDEX]
        # CHILD: IT'S THE LEFT CHILD IF FEATURE_VALUE <= CURRENT_NODE.THRESHOLD, OTHERWISE IT'S THE RIGHT CHILD.
        CHILD = CURRENT_NODE.LEFT if FEATURE_VALUE <= CURRENT_NODE.THRESHOLD else CURRENT_NODE.RIGHT
