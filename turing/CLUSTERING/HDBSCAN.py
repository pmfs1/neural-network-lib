...
def HDBSCAN(X: np.ndarray, MIN_SAMPLES: int = 5) -> np.ndarray:
    ...
    def __COMPUTE_CORE_DISTANCES__(X: np.ndarray) -> np.ndarray:
    ...
    def __COMPUTE_NEIGHBOURS__(X: np.ndarray, CORE_DISTANCES: np.ndarray) -> np.ndarray:
    ...
    def __CONDENSE_TREE__(X: np.ndarray, NEIGHBOURS: np.ndarray, CORE_DISTANCES: np.ndarray) -> dict:
    ...
    def __EXTRACT_CLUSTERS__(CONDENSED_TREE: dict, N_SAMPLES: int) -> np.ndarray:
        ...
        for EDGE, _ in sorted(CONDENSED_TREE.items(), key=lambda x: x[1]):
            ...
            # IF BOTH SAMPLES HAVE ASSIGNED LABELS, IT MEANS THAT THEY ARE PART OF DIFFERENT CLUSTERS. IN THIS CASE, MERGE THE TWO CLUSTERS BY ASSIGNING A COMMON LABEL TO ALL SAMPLES IN THE CONNECTED CLUSTERS.
                ...
                # LABELS[CLUSTER_A] < LABELS[CLUSTER_B]
                if LABELS[CLUSTER_A] < LABELS[CLUSTER_B]:
                    # MERGE LABEL IS THE SMALLER OF THE TWO LABELS.
                    MERGE_LABEL = LABELS[CLUSTER_A]
                else:  # LABELS[CLUSTER_A] > LABELS[CLUSTER_B]
                    # MERGE LABEL IS THE SMALLER OF THE TWO LABELS.
                    MERGE_LABEL = LABELS[CLUSTER_B]
    ...
    def __POST_PROCESS_LABELS__(LABELS: np.ndarray) -> np.ndarray:
        """THE `__POST_PROCESS_LABELS__` FUNCTION IS A HELPER FUNCTION WITHIN THE HDBSCAN ALGORITHM THAT PERFORMS A POST-PROCESSING STEP ON THE CLUSTER LABELS OBTAINED FROM THE CONDENSED TREE.
            1. UNIQUE LABELS: THE FUNCTION IDENTIFIES THE UNIQUE CLUSTER LABELS PRESENT IN THE INPUT ARRAY OF CLUSTER LABELS.
            2. RELABELLING: IT ASSIGNS NEW LABELS TO CLUSTERS, ENSURING SEQUENTIAL AND ZERO-BASED INDEXING. THE FUNCTION ITERATES OVER THE UNIQUE LABELS AND ASSIGNS NEW LABEL INDICES TO SAMPLES WITH THE SAME LABEL.
            3. UPDATED LABELS: THE FUNCTION GENERATES AN ARRAY OF CLUSTER LABELS WITH THE UPDATED AND RELABELED VALUES.
            4. OUTPUT: THE FUNCTION RETURNS THE ARRAY OF UPDATED CLUSTER LABELS AS THE OUTPUT.
        IN SUMMARY, THE `__POST_PROCESS_LABELS__` FUNCTION PERFORMS A POST-PROCESSING STEP TO REFINE THE CLUSTER LABELS OBTAINED FROM THE CONDENSED TREE. IT ENSURES THAT THE LABELS ARE SEQUENTIALLY INDEXED STARTING FROM ZERO, PROVIDING A CONSISTENT AND MORE INTERPRETABLE LABELING SCHEME. THE RESULTING ARRAY OF UPDATED CLUSTER LABELS IS RETURNED AS THE OUTPUT OF THE FUNCTION. THIS POST-PROCESSING STEP IMPROVES THE CLARITY AND USABILITY OF THE CLUSTERING RESULTS."""
