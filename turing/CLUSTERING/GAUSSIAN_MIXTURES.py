...
def GAUSSIAN_MIXTURES(X: np.ndarray, K: int = 2, MAX_ITERATIONS: int = 100, TOLERANCE: float = 1E-4) -> np.ndarray:
    ...
    def __ESTIMATE_RESPONSIBILITIES__(X: np.ndarray, WEIGHTS: np.ndarray, MEANS: np.ndarray, COVARIANCES: np.ndarray) -> np.ndarray:
        """THE `__ESTIMATE_RESPONSIBILITIES__` FUNCTION IS A HELPER FUNCTION WITHIN THE `GAUSSIAN_MIXTURES` FUNCTION. ITS PURPOSE IS TO COMPUTE THE RESPONSIBILITIES, WHICH REPRESENT THE PROBABILITIES THAT EACH DATA SAMPLE BELONGS TO EACH GAUSSIAN COMPONENT.
            - INPUTS:
                - `X`: THE INPUT DATA REPRESENTED AS A NUMPY ARRAY, WHERE EACH ROW CORRESPONDS TO A DATA SAMPLE AND EACH COLUMN REPRESENTS A FEATURE.
                - `WEIGHTS`: THE WEIGHTS OF THE GAUSSIAN COMPONENTS.
        ...
        for i in range(K):  # ITERATE OVER COMPONENTS
            WEIGHTED_DENSITIES[:, i] = WEIGHTS[i] * __MULTIVARIATE_NORMAL__(
                X, MEANS[i], COVARIANCES[i])  # COMPUTE WEIGHTED DENSITIES
        RESPONSIBILITY_MATRIX = WEIGHTED_DENSITIES / \
            np.sum(WEIGHTED_DENSITIES, axis=1,
                   keepdims=True)  # COMPUTE RESPONSIBILITY MATRIX
    ...
    def __ESTIMATE_PARAMETERS__(X: np.ndarray, RESPONSIBILITY_MATRIX: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        # SUM OF RESPONSIBILITIES ACROSS ALL SAMPLES FOR EACH COMPONENT
        RESPONSIBILITY_MATRIX_SUM = np.sum(RESPONSIBILITY_MATRIX, axis=0)
        WEIGHTS = RESPONSIBILITY_MATRIX_SUM / N_SAMPLES  # UPDATE WEIGHTS
        MEANS = np.dot(RESPONSIBILITY_MATRIX.T, X) / \
            RESPONSIBILITY_MATRIX_SUM[:, np.newaxis]  # UPDATE MEANS
        COVARIANCES = []  # INITIALIZE LIST OF COVARIANCES
        for i in range(K):  # ITERATE OVER EACH COMPONENT
            # DIFFERENCE BETWEEN DATA SAMPLES AND MEANS
            DIFFERENCE = X - MEANS[i]
            # WEIGHTED DIFFERENCES
            WEIGHTED_DIFFERENCE = RESPONSIBILITY_MATRIX[:,
                                                        i][:, np.newaxis] * DIFFERENCE
            COVARIANCES.append(np.dot(WEIGHTED_DIFFERENCE.T, DIFFERENCE) /
                               RESPONSIBILITY_MATRIX_SUM[i])  # UPDATE COVARIANCE MATRICES
        # CONVERT LIST OF COVARIANCES TO NUMPY ARRAY
        COVARIANCES = np.array(COVARIANCES)
        # RETURN UPDATED WEIGHTS, MEANS, AND COVARIANCES
    ...
    def __MULTIVARIATE_NORMAL__(X: np.ndarray, MEAN: np.ndarray, COVARIANCE_MATRIX: np.ndarray) -> np.ndarray:
        ...
        NORMALIZATION_CONSTANT = 1.0 / \
            ((2 * np.pi) ** (K / 2) * np.sqrt(DETERMINANT))  # NORMALIZATION CONSTANT
        INVERSE_OF_COVARIANCE_MATRIX = np.linalg.inv(
            COVARIANCE_MATRIX)  # INVERSE OF COVARIANCE MATRIX
        # EXPONENT OF MULTIVARIATE NORMAL DISTRIBUTION
        EXPONENT = -0.5 * \
            np.sum(np.dot((X - MEAN), INVERSE_OF_COVARIANCE_MATRIX)
                   * (X - MEAN), axis=1)
    ...
    def __LOG_LIKELIHOOD__(X: np.ndarray, WEIGHTS: np.ndarray, MEANS: np.ndarray, COVARIANCES: np.ndarray) -> float:
        ...
        LOG_LIKELIHOODS = np.log(np.sum([WEIGHTS[i] * __MULTIVARIATE_NORMAL__(X, MEANS[i], COVARIANCES[i]) for i in range(WEIGHTS.shape[0])], axis=0)
                                 )  # DETERMINE THE LOG-LIKELIHOOD OF THE GMM BY ITERATING OVER THE GAUSSIAN COMPONENTS, COMPUTING THE PROBABILITY DENSITY VALUES, WEIGHTING THEM BY THE COMPONENT WEIGHTS, AND CALCULATING THE LOG-LIKELIHOOD BASED ON THE SUM OF THESE WEIGHTED PROBABILITY DENSITY VALUES
        return np.sum(LOG_LIKELIHOODS)  # RETURN THE LOG-LIKELIHOOD

    # EXTRACT THE NUMBER OF SAMPLES AND FEATURES FROM THE SHAPE OF THE INPUT DATA
    N_SAMPLES, N_FEATURES = X.shape
    WEIGHTS = np.ones(K) / K  # INITIALIZE THE COMPONENT WEIGHTS TO BE UNIFORM
    # INITIALIZE THE COMPONENT MEANS TO BE RANDOM SAMPLES FROM THE INPUT DATA
    MEANS = X[np.random.choice(N_SAMPLES, K, replace=False)]
    # INITIALIZE THE COMPONENT COVARIANCE MATRICES TO BE IDENTITY MATRICES
    ...
    for _ in range(MAX_ITERATIONS):  # ITERATE OVER THE MAXIMUM NUMBER OF ITERATIONS
        RESPONSIBILITY_MATRIX = __ESTIMATE_RESPONSIBILITIES__(
            X, WEIGHTS, MEANS, COVARIANCES)  # ESTIMATE THE RESPONSIBILITY MATRIX
        WEIGHTS, MEANS, COVARIANCES = __ESTIMATE_PARAMETERS__(
            X, RESPONSIBILITY_MATRIX)  # ESTIMATE THE PARAMETERS
        LOG_LIKELIHOOD = __LOG_LIKELIHOOD__(
            X, WEIGHTS, MEANS, COVARIANCES)  # ESTIMATE THE LOG-LIKELIHOOD
        # IF THE DIFFERENCE BETWEEN THE LOG-LIKELIHOOD AND THE PREVIOUS LOG-LIKELIHOOD IS LESS THAN THE TOLERANCE
    ...
    RESPONSIBILITY_MATRIX = __ESTIMATE_RESPONSIBILITIES__(
        X, WEIGHTS, MEANS, COVARIANCES)  # ESTIMATE THE RESPONSIBILITY MATRIX
    # DETERMINE THE LABELS BY TAKING THE ARGUMENTS OF THE MAXIMUM VALUES IN THE RESPONSIBILITY MATRIX
