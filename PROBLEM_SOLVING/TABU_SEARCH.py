...
#     1. THERE ARE TWO HELPER FUNCTIONS DEFINED WITHIN THE `SIMULATED_ANNEALING` FUNCTION:
#         - THE `__INITIAL_STATE__` FUNCTION IS A HELPER FUNCTION THAT INITIALIZES THE INITIAL STATE FOR SOLVING THE CSP. IT CREATES A DICTIONARY CALLED `INITIAL_STATE` WHERE EACH VARIABLE IS ASSIGNED THE FIRST VALUE FROM ITS DOMAIN.
#         - THE `__SOLUTION__` FUNCTION IS A HELPER FUNCTION THAT CHECKS WHETHER A GIVEN STATE SATISFIES ALL THE CONSTRAINTS OF THE CSP. IT ITERATES OVER EACH CONSTRAINT AND USES THE `__CONSTRAINT_SATISFIED__` FUNCTION TO DETERMINE IF THE CONSTRAINT IS SATISFIED. IF ANY CONSTRAINT IS NOT SATISFIED, IT RETURNS FALSE. OTHERWISE, IT RETURNS TRUE.
#         - THE `__CONSTRAINT_SATISFIED__` FUNCTION IS A HELPER FUNCTION THAT CHECKS WHETHER A GIVEN STATE SATISFIES A SPECIFIC CONSTRAINT IN THE CSP. IT TAKES A STATE DICTIONARY AND A CONSTRAINT TUPLE AS INPUT. THE CONSTRAINT TUPLE REPRESENTS A CONSTRAINT BETWEEN TWO VARIABLES. IT CHECKS IF THE VALUES ASSIGNED TO THE VARIABLES IN THE GIVEN STATE SATISFY THE CONSTRAINT. IF THE VALUES ARE EQUAL, IT MEANS THE CONSTRAINT IS VIOLATED, AND IT RETURNS FALSE. OTHERWISE, IT RETURNS TRUE.
#         - THE `__BEST_SUCCESSOR__` FUNCTION IS A HELPER FUNCTION THAT SELECTS THE BEST SUCCESSOR STATE FROM A SET OF GENERATED SUCCESSORS. IT TAKES THE CURRENT STATE AND THE TABU LIST AS INPUT. IT ITERATES OVER EACH SUCCESSOR STATE GENERATED BY THE `__SUCCESSORS__` FUNCTION. IF A SUCCESSOR STATE IS NOT IN THE TABU LIST, IT COMPARES IT WITH THE CURRENT BEST SUCCESSOR (IF ANY) BASED ON A COST FUNCTION DEFINED IN THE `__BETTER__` FUNCTION. THE SUCCESSOR WITH A LOWER COST IS CONSIDERED BETTER. FINALLY, IT RETURNS THE BEST SUCCESSOR STATE.
#         - THE `__SUCCESSORS__` FUNCTION IS A HELPER FUNCTION THAT GENERATES A SET OF SUCCESSOR STATES FROM A GIVEN STATE IN THE CSP. IT ITERATES OVER EACH VARIABLE IN THE CSP AND FOR EACH VARIABLE, IT GENERATES A SUCCESSOR STATE BY ASSIGNING DIFFERENT VALUES FROM THE DOMAIN OF THAT VARIABLE WHILE KEEPING THE OTHER VARIABLES' VALUES THE SAME. THESE SUCCESSOR STATES ARE ADDED TO A LIST CALLED `SUCCESSORS`, WHICH IS RETURNED AT THE END.
#         - THE `__BETTER__` FUNCTION IS A HELPER FUNCTION THAT COMPARES TWO SUCCESSOR STATES AND DETERMINES WHICH ONE IS CONSIDERED "BETTER" BASED ON A COST FUNCTION. IT TAKES TWO SUCCESSOR STATES AS INPUT AND COMPARES THEIR COSTS USING THE `__COST__` FUNCTION. IF THE COST OF THE FIRST SUCCESSOR IS LOWER THAN THE COST OF THE SECOND SUCCESSOR, IT RETURNS TRUE INDICATING THAT THE FIRST SUCCESSOR IS BETTER. OTHERWISE, IT RETURNS FALSE.
#         - THE `__COST__` FUNCTION IS A HELPER FUNCTION THAT CALCULATES THE COST OF A GIVEN STATE IN THE CSP. IT ITERATES OVER EACH CONSTRAINT IN THE CSP AND USES THE `__CONSTRAINT_SATISFIED__` FUNCTION TO CHECK IF THE CONSTRAINT IS VIOLATED. FOR EACH VIOLATED CONSTRAINT, IT INCREMENTS THE COST BY 1. THE FINAL COST IS RETURNED.
#     2. THE MAIN PART OF THE `TABU_SEARCH` FUNCTION STARTS BY INITIALIZING THE CURRENT STATE USING THE `__INITIAL_STATE__` FUNCTION. IT ALSO INITIALIZES AN EMPTY TABU LIST AND SETS THE TABU LIST SIZE TO THE NUMBER OF VARIABLES IN THE CSP.
#     3. THE FUNCTION ENTERS A WHILE LOOP THAT CONTINUES UNTIL A SOLUTION IS FOUND OR THERE ARE NO MORE BEST SUCCESSOR STATES. IN EACH ITERATION, IT CHECKS IF THE CURRENT STATE IS A SOLUTION BY CALLING THE `__SOLUTION__` FUNCTION. IF IT IS, THE CURRENT STATE IS RETURNED AS THE SOLUTION.
#     4. IF THE CURRENT STATE IS NOT A SOLUTION, THE FUNCTION SELECTS THE BEST SUCCESSOR STATE USING THE `__BEST_SUCCESSOR__` FUNCTION, PASSING IN THE CURRENT STATE AND THE TABU LIST. IF THERE IS NO BEST SUCCESSOR STATE (ALL SUCCESSORS ARE IN THE TABU LIST), THE FUNCTION RETURNS NONE INDICATING THAT NO SOLUTION COULD BE FOUND.
#     5. THE BEST SUCCESSOR STATE IS ADDED TO THE TABU LIST, AND IF THE TABU LIST SIZE EXCEEDS THE PREDEFINED TABU LIST SIZE, THE OLDEST ENTRY IS REMOVED FROM THE LIST.
#     6. THE LOOP CONTINUES TO THE NEXT ITERATION WITH THE UPDATED CURRENT STATE, AND THE PROCESS REPEATS UNTIL A SOLUTION IS FOUND OR NO MORE BEST SUCCESSOR STATES ARE AVAILABLE.
# THAT'S THE EXPLANATION OF THE `TABU_SEARCH` FUNCTION AND ITS HELPER FUNCTIONS. THE FUNCTION USES A TABU LIST TO KEEP TRACK OF PREVIOUSLY VISITED STATES AND AVOIDS GETTING STUCK IN LOCAL OPTIMA BY ALLOWING MOVEMENTS TO SUBOPTIMAL STATES.
