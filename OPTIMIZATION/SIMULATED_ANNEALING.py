...
def SIMULATED_ANNEALING(CSP_PROBLEM: CSP) -> dict:
    ...
    while TEMPERATURE > 1:  # LOOP UNTIL THE TEMPERATURE DROPS BELOW 1
        ...
        if __COST_FUNCTION__(CSP_PROBLEM, NEXT_STATE) < __COST_FUNCTION__(CSP_PROBLEM, CURRENT_STATE):
            ...
            PROBABILITY = math.exp(- (__COST_FUNCTION__(CSP_PROBLEM, NEXT_STATE) - __COST_FUNCTION__(
                CSP_PROBLEM, CURRENT_STATE)) / TEMPERATURE)  # CALCULATE THE PROBABILITY OF ACCEPTING THE NEXT STATE
...
#     1. IT TAKES TWO INPUTS: `CSP_PROBLEM`, WHICH REPRESENTS THE CSP BEING SOLVED AND CONTAINS INFORMATION ABOUT THE VARIABLES, DOMAINS, AND CONSTRAINTS, AND `STATE`, WHICH IS A DICTIONARY REPRESENTING AN ASSIGNMENT OF VALUES TO VARIABLES IN THE CSP.
...
def __COST_FUNCTION__(CSP_PROBLEM: CSP, STATE: dict) -> int:
    """THE `__COST_FUNCTION__` IS A HELPER FUNCTION USED BY THE `SIMULATED_ANNEALING` ALGORITHM TO CALCULATE THE COST OF A GIVEN ASSIGNMENT (STATE) IN A CONSTRAINT SATISFACTION PROBLEM (CSP)."""
