...
#     1. `CURRENT_STATE = __INITIAL_STATE__(CSP_PROBLEM)`: THIS LINE INITIALIZES THE CURRENT STATE BY CALLING THE `__INITIAL_STATE__` FUNCTION, WHICH CREATES A DICTIONARY REPRESENTING THE INITIAL ASSIGNMENT OF VARIABLES.
...
#     5. `IF __SOLUTION__(CURRENT_STATE, CSP_PROBLEM):`: THIS CONDITION CHECKS IF THE CURRENT STATE IS A SOLUTION BY CALLING THE `__SOLUTION__` FUNCTION, WHICH VERIFIES IF ALL CONSTRAINTS IN THE CSP PROBLEM ARE SATISFIED. IF A SOLUTION IS FOUND, THE CURRENT STATE IS RETURNED.
#     6. `CURRENT_STATE = __BEST_SUCCESSOR__(CURRENT_STATE, CSP_PROBLEM, TABU_LIST)`: IF THE CURRENT STATE IS NOT A SOLUTION, THE ALGORITHM GENERATES THE BEST SUCCESSOR STATE BY CALLING THE `__BEST_SUCCESSOR__` FUNCTION. THIS FUNCTION EXAMINES ALL POSSIBLE SUCCESSORS OF THE CURRENT STATE, EXCLUDING THOSE IN THE TABU LIST, AND SELECTS THE BEST ONE BASED ON A COST FUNCTION.
...
def TABU_SEARCH(CSP_PROBLEM: CSP) -> dict | None:
    ...
    CURRENT_STATE = __INITIAL_STATE__(CSP_PROBLEM)  # INITIALIZE CURRENT STATE
    ...
    while True:  # LOOP UNTIL SOLUTION IS FOUND OR NO MORE VALID SUCCESSORS CAN BE GENERATED
        if __SOLUTION__(CURRENT_STATE, CSP_PROBLEM):  # IF CURRENT STATE IS A SOLUTION
            return CURRENT_STATE  # RETURN CURRENT STATE
        CURRENT_STATE = __BEST_SUCCESSOR__(
            CURRENT_STATE, CSP_PROBLEM, TABU_LIST)  # GENERATE BEST SUCCESSOR
...
def __INITIAL_STATE__(CSP_PROBLEM: CSP) -> dict:
    """THE `__INITIAL_STATE__()` FUNCTION IS A HELPER FUNCTION USED IN THE TABU SEARCH ALGORITHM TO INITIALIZE THE INITIAL STATE FOR SOLVING A CONSTRAINT SATISFACTION PROBLEM (CSP)."""
...
def __SOLUTION__(STATE: dict, CSP_PROBLEM: CSP) -> bool:
    """THE `__SOLUTION__()` FUNCTION IS A HELPER FUNCTION USED IN THE TABU SEARCH ALGORITHM TO DETERMINE WHETHER A GIVEN STATE SATISFIES ALL THE CONSTRAINTS OF A CONSTRAINT SATISFACTION PROBLEM (CSP)."""
...
#     2. `FOR SUCCESSOR IN __SUCCESSORS__(STATE, CSP_PROBLEM):`: THIS LINE STARTS A LOOP THAT ITERATES OVER EACH SUCCESSOR STATE GENERATED FROM THE CURRENT STATE. IT CALLS THE `__SUCCESSORS__()` FUNCTION TO GENERATE THE SUCCESSORS, PASSING THE CURRENT STATE `STATE` AND THE CSP PROBLEM `CSP_PROBLEM` AS ARGUMENTS.
...
#     4. `IF BEST_SUCCESSOR IS NONE OR __BETTER__(SUCCESSOR, BEST_SUCCESSOR, CSP_PROBLEM): BEST_SUCCESSOR = SUCCESSOR: IF `BEST_SUCCESSOR` IS STILL `NONE`, INDICATING THAT NO BEST SUCCESSOR HAS BEEN ASSIGNED YET, OR IF `BEST_SUCCESSOR` IS NOT `NONE`, THE CONDITION `__BETTER__(SUCCESSOR, BEST_SUCCESSOR, CSP_PROBLEM)` IS CHECKED. IT CALLS THE `__BETTER__()` FUNCTION TO DETERMINE IF THE CURRENT SUCCESSOR IS BETTER THAN THE CURRENT BEST SUCCESSOR. IF IT IS, THE CURRENT SUCCESSOR STATE REPLACES THE BEST SUCCESSOR.
...
def __BEST_SUCCESSOR__(STATE: dict, CSP_PROBLEM: CSP, TABU_LIST: list) -> dict | None:
    """THE `__BEST_SUCCESSOR__()` FUNCTION IS A HELPER FUNCTION USED IN THE TABU SEARCH ALGORITHM TO SELECT THE BEST SUCCESSOR STATE FROM A SET OF GENERATED SUCCESSORS."""
    ...
    for SUCCESSOR in __SUCCESSORS__(STATE, CSP_PROBLEM):
        if SUCCESSOR not in TABU_LIST:  # IF SUCCESSOR NOT IN TABU LIST
            ...
            if BEST_SUCCESSOR is None or __BETTER__(SUCCESSOR, BEST_SUCCESSOR, CSP_PROBLEM):
...
def __SUCCESSORS__(STATE: dict, CSP_PROBLEM: CSP) -> list:
    """THE `__SUCCESSORS__()` FUNCTION IS A HELPER FUNCTION USED IN THE TABU SEARCH ALGORITHM TO GENERATE A SET OF SUCCESSOR STATES FROM A GIVEN STATE IN A CONSTRAINT SATISFACTION PROBLEM (CSP)."""
...
def __BETTER__(SUCCESSOR: dict, BEST_SUCCESSOR: dict, CSP_PROBLEM: CSP) -> bool:
    """THE `__BETTER__()` FUNCTION IS A HELPER FUNCTION USED IN THE TABU SEARCH ALGORITHM TO COMPARE TWO SUCCESSOR STATES AND DETERMINE WHICH ONE IS CONSIDERED "BETTER" BASED ON A COST FUNCTION."""
...
def __COST__(STATE: dict, CSP_PROBLEM: CSP) -> int:
    """THE `__COST__()` FUNCTION IS A HELPER FUNCTION USED IN THE TABU SEARCH ALGORITHM TO CALCULATE THE COST OF A GIVEN STATE IN A CONSTRAINT SATISFACTION PROBLEM (CSP)."""
