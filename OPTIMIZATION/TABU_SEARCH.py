...
def TABU_SEARCH(CSP_PROBLEM: CSP):
    """RUNS THE TABU SEARCH ALGORITHM IN ORDER TO FIND A SOLUTION TO THE CONSTRAINT SATISFACTION PROBLEM. RETURNS THE CURRENT STATE IF A SOLUTION IS FOUND, OTHERWISE RETURNS FALSE.
    ...
    while True:
        ...
        CURRENT_STATE = __BEST_SUCCESSOR__(
            CURRENT_STATE, CSP_PROBLEM, TABU_LIST)
...
def __INITIAL_STATE__(CSP_PROBLEM: CSP):
    ...
    INITIAL_STATE : DICTIONARY
        THE INITIAL STATE. IS THE FIRST STATE TO BE CONSIDERED BY THE TABU SEARCH ALGORITHM. ITS VALUE IS THE FIRST VALUE IN THE DOMAIN OF EACH VARIABLE.
    """
...
def __SOLUTION__(STATE, CSP_PROBLEM: CSP):
    ...
    ----------
    STATE : DICTIONARY
        THE STATE TO BE CHECKED. IT REPRESENTS A SOLUTION TO THE CONSTRAINT SATISFACTION PROBLEM.
    CSP_PROBLEM : CSP
        THE CONSTRAINT SATISFACTION PROBLEM.
...
def __CONSTRAINT_SATISFIED__(STATE, CONSTRAINT):
    ...
    STATE : DICTIONARY
        THE STATE TO BE CHECKED. IT REPRESENTS A SOLUTION TO THE CONSTRAINT SATISFACTION PROBLEM.
    CONSTRAINT : LIST
        THE CONSTRAINT TO BE CHECKED. IT IS A LIST OF PAIRS OF VARIABLES.
...
def __BEST_SUCCESSOR__(STATE, CSP_PROBLEM: CSP, TABU_LIST):
    """[PRIVATE FUNCTION]: RETURNS THE BEST SUCCESSOR OF THE STATE, OTHERWISE RETURNS FALSE.
    ...
    CSP_PROBLEM : CSP
        THE CONSTRAINT SATISFACTION PROBLEM.
    TABU_LIST : LIST
        THE TABU LIST. IT IS A LIST OF STATES. IT STORES THE STATES THAT HAVE BEEN VISITED.

    RETURNS
    -------
    BEST_SUCCESSOR : DICTIONARY
        THE BEST SUCCESSOR OF THE STATE. IT IS THE BEST STATE TO BE CONSIDERED BY THE TABU SEARCH ALGORITHM. ITS VALUE IS THE BEST VALUE IN THE DOMAIN OF EACH VARIABLE.
    """
...
def __SUCCESSORS__(STATE, CSP_PROBLEM: CSP):
    ...
    -------
    SUCCESSORS : LIST
        THE SUCCESSORS OF THE STATE. THEY ARE THE STATES THAT CAN BE REACHED FROM THE STATE.
    """
...
def __BETTER__(SUCCESSOR, BEST_SUCCESSOR, CSP_PROBLEM: CSP):
    ...
    SUCCESSOR : DICTIONARY
        THE SUCCESSOR TO BE CHECKED.
    BEST_SUCCESSOR : DICTIONARY
        THE BEST SUCCESSOR TO BE CHECKED.
    CSP_PROBLEM : CSP
        THE CONSTRAINT SATISFACTION PROBLEM.
...
def __COST__(STATE, CSP_PROBLEM: CSP):
    """[PRIVATE FUNCTION]: RETURNS THE COST OF THE STATE.
    ...
    -------
    COST : INTEGER
        THE COST OF THE STATE. IT IS THE NUMBER OF CONSTRAINTS THAT ARE NOT SATISFIED.
    """
    # INITIALIZE THE COST.
    COST = 0
