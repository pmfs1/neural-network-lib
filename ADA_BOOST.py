...
class DECISION_STUMP:
    ...
    # FIT_PREDICT(): METHOD THAT TRAINS AND PREDICTS THE DECISION STUMP MODEL
    def FIT_PREDICT(self, X):
        ...
        if self.POLARITY == 1:  # IF THE POLARITY IS 1
            ...
            PREDICTIONS[X_COLUMN > self.THRESHOLD] = -1
        return PREDICTIONS  # RETURNS THE PREDICTED DATASET
...
class ADA_BOOST:
    ...
    # PREDICT(): METHOD THAT PREDICTS THE DATA
    def PREDICT(self, X):
        # CLF_PREDS: IT'S THE ARRAY THAT CONTAINS THE PREDICTIONS OF THE CLASSIFIERS.
        CLF_PREDS = [CLF.ALPHA * CLF.FIT_PREDICT(X) for CLF in self.CLFS]
        # PREDICTION: IT'S THE ARRAY THAT CONTAINS THE PREDICTION OF THE MODEL.
        PREDICTION = np.sum(CLF_PREDS, axis=0)
        return np.sign(PREDICTION)  # RETURNS THE SIGN OF THE PREDICTION
